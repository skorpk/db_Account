...
DECLARE @dateStart DATETIME='20190101',	--всегда с начало года
		@dateEnd DATETIME='20190406',
		@reportYear SMALLINT=2019,
		@dateEndAkt DATETIME='20190406'		
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=u.rf_idCase                  
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=@reportYear AND c.rf_idV006<4 AND c.rf_idV002=60 AND a.rf_idSMO<>'34'
-------------------------Колонки 33-38----------------------------------
...
FROM #tCases2 c INNER JOIN dbo.tmp_PaymentSkrybina p ON
		c.rf_idCase=p.rf_idCase
...
FROM (
	...
	FROM #tCases2 c INNER JOIN dbo.tmp_PaymentSkrybina p ON
			c.rf_idCase=p.rf_idCase
	WHERE p.DateRegistration>@dateStart AND p.DateRegistration<@dateEnd AND p.TypeCheckup IN(20,21,22,23)
...
FROM (
	...
	FROM #tCases2 c INNER JOIN dbo.tmp_PaymentSkrybina p ON
			c.rf_idCase=p.rf_idCase
	WHERE p.DateRegistration>@dateStart AND p.DateRegistration<@dateEnd AND p.TypeCheckup IN(20,21,22,23) AND c.rf_idN013=2
	GROUP BY c.rf_idCase,c.AmountPay
	) t
WHERE t.AmountPay>0 
---------------------EKMP----------------------------------
...
FROM (
	...
	FROM #tCases2 c INNER JOIN dbo.tmp_PaymentSkrybina p ON
			c.rf_idCase=p.rf_idCase
	WHERE p.DateRegistration>@dateStart AND p.DateRegistration<@dateEnd AND p.TypeCheckup >29 AND p.TypeCheckup<39
	GROUP BY c.rf_idCase,c.AmountPay
	)t
WHERE t.AmountPay>0
...
FROM (
	...
	FROM #tCases2 c INNER JOIN dbo.tmp_PaymentSkrybina p ON
			c.rf_idCase=p.rf_idCase
	WHERE p.DateRegistration>@dateStart AND p.DateRegistration<@dateEnd AND p.TypeCheckup >29 AND p.TypeCheckup<39 AND c.rf_idN013=2
	GROUP BY c.rf_idCase,c.AmountPay
	)t
WHERE t.AmountPay>0
-------------------------Колонки 49-54----------------------------------
select p.rf_idCase,p.AmountPay-r.AmountDeduction AS AmountPay
INTO #tCaseMEE
FROM #tCases2 p INNER JOIN (SELECT c.rf_idCase,SUM(c.AmountDeduction) AS AmountDeduction
								FROM dbo.tmp_PaymentSkrybina c
								WHERE c.DateRegistration>=@dateStart AND c.DateRegistration<@dateEndAkt AND c.TypeCheckup IN(20,21,22,23)	 
								GROUP BY c.rf_idCase
							) r ON
			p.rf_idCase=r.rf_idCase
WHERE p.AmountPay-r.AmountDeduction>0

select p.rf_idCase,p.AmountPay-r.AmountDeduction AS AmountPay
INTO #tCaseEKMP
FROM #tCases2 p INNER JOIN (SELECT c.rf_idCase,SUM(c.AmountDeduction) AS AmountDeduction
								FROM dbo.tmp_PaymentSkrybina c
								WHERE c.DateRegistration>=@dateStart AND c.DateRegistration<@dateEndAkt AND c.TypeCheckup >29 AND c.TypeCheckup<39	 
								GROUP BY c.rf_idCase
							) r ON
			p.rf_idCase=r.rf_idCase
WHERE p.AmountPay-r.AmountDeduction>0
