USE AccountOMS
GO
alter PROCEDURE usp_ExportCanserCases
as
...
INSERT dbo.tmp_PeopleEnp( ENP ,Fam ,Im ,Ot ,Dr ,Sex ,TypeSearch)
...
INSERT dbo.tmp_PeopleDS_ONK ( ENP, DateDS_ONK, DS_ONK,rf_idCase )
...
INSERT dbo.tmp_PeopleDiagnosis( ENP, DiagnosisCode, DateSetup,rf_idCase )
SELECT ENP,DiagnosisCode,DateEnd,rf_idCase FROM cteTotal WHERE idRow=1

------Стадия----------
INSERT dbo.tmp_PeopleSTAD( ENP, SATD, rf_idCase )
SELECT ENP,o.rf_idN002 AS STAD,d.rf_idCase
FROM tmp_PeopleDiagnosis d INNER JOIN dbo.t_ONK_SL o ON
...
;WITH cteN13
AS(
SELECT ROW_NUMBER() OVER(PARTITION BY ps.ENP ORDER BY cc.DateEnd desc) AS idRow, ps.ENP,cc.DateEnd, u.rf_idN013 AS usl_tip ,c.id AS rf_idCase
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
				f.id=a.rf_idFiles
						INNER JOIN dbo.t_RecordCasePatient r ON
				a.id=r.rf_idRegistersAccounts					
...
INSERT dbo.tmp_PeopleSMO( ENP,CodeSMO,rf_idCase)
...
INSERT dbo.tmp_PeopleNAPR( ENP ,NAPR_DATE ,rf_idCase ,rf_idV029)
...
from PolicyRegister.dbo.polis s join PolicyRegister.dbo.people p ON 
						...
						p.ENP=t.ENP
where s.dbeg <= GETDATE() AND p.DS IS NULL
)
INSERT dbo.tmp_PeopleEND( ENP, DateEnd, typeEnd )
 SELECT  ENP,DSTOP,1
