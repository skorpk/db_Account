...
SELECT e.id,e.ENP, e.Fam+' '+e.Im+' '+ISNULL(e.Ot,'') AS FIO, BirthYear, e.Sex
		...
		,sm.CodeSMO
		,st.DateRegistration 
FROM dbo.t_PeopleEnp e inner JOIN vw_PeopleDateStart st ON
			e.id=st.rf_idPeopleENP
						LEFT JOIN dbo.t_PeopleDS_ONK d ON	
			...
			e.id=s.rf_idPeopleENP                   
						LEFT JOIN dbo.vw_PeoplePCEL pc ON
...
GROUP BY e.id,e.ENP, e.Fam+' '+e.Im+' '+ISNULL(e.Ot,'') , BirthYear, e.Sex, CASE WHEN d.DS_ONK=1 THEN 1 ELSE 0 END , CASE WHEN d.DS_ONK=1 THEN d.DateDS_ONK ELSE NULL END 
		...
		,CASE WHEN pe.typeEnd=1 THEN 'Отсутствие страхование в ВО' WHEN pe.typeEnd=2 THEN 'Смерть' ELSE NULL END ,ISNULL(typeEnd,0) ,sm.CodeSMO,st.DateRegistration 
GO
