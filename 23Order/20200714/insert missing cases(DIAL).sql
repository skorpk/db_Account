...
FROM dbo.t_Case c INNER JOIN dbo.t_CompletedCase cc ON
			c.rf_idRecordCasePatient=cc.rf_idRecordCasePatient                  
WHERE c.id IN(116545619,116546169,116546177,116546183,116546187,116660430,116763185)
/*
09.02.2019
...
/*
	...
	SELECT distinct c.id,c.rf_idMO ,c.rf_idV006 ,c.rf_idV002, c.DateBegin,c.DateEnd,c.AmountPayment,
	 CAST(c.rf_idDepartmentMO AS VARCHAR(6)), 10,CASE WHEN c.rf_idV010=43 THEN 33 ELSE c.rf_idV010 END, 0.0
...
FROM #tPeople p INNER JOIN (
							...
							WHERE p.DateRegistration>='20200610' AND p.DateRegistration<@dateEndPay	 
...
FROM UnitOfHosp u INNER JOIN dbo.t_Case c ON
		...
		mes.MES=csg.code	
		AND cc.DateEnd BETWEEN csg.dateBeg AND csg.dateEnd
					INNER JOIN (SELECT distinct id,rf_idCase,MUSurgery,Quantity,TotalPrice FROM dbo.t_Meduslugi WHERE MUInt=0) mu1 ON
		c.id=mu1.rf_idCase       
