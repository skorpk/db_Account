...
SELECT DISTINCT c.id AS rf_idCase,c.rf_idMO AS CodeM,a.rf_idMO,a.ReportMonth,a.ReportYear,r.rf_idF008,c.rf_idV006,r.SeriaPolis,r.NumberPolis,p.BirthDay,p.rf_idV005
		...
		,c.Age,CASE WHEN c.Age>0 AND c.Age<4 THEN 4
					...
					WHEN c.Age>59 AND c.Age<76 THEN 7
...
SELECT DISTINCT c.id AS rf_idCase,c.rf_idMO AS CodeM,a.rf_idMO,a.ReportMonth,a.ReportYear,r.rf_idF008,c.rf_idV006,r.SeriaPolis,r.NumberPolis,p.BirthDay,p.rf_idV005
		...
		,'DIAL' AS K_KSG
		,0 AS KSG_PG
		,c.IT_SL,ps.ENP,TypeCases,CAST(m.Quantity AS INT) AS Quantity, m.TotalPrice,NULL AS NoLevelCoefficient
FROM UnitOfHosp u INNER JOIN dbo.t_Case c ON
		u.rf_idCase=c.id
					INNER JOIN dbo.t_RecordCasePatient r ON
		r.id=c.rf_idRecordCasePatient
...
INSERT dbo.t_SendingDataIntoFFOMS( id ,rf_idCase ,CodeM ,rf_idMO ,ReportMonth ,ReportYear ,rf_idF008 ,rf_idV006 ,SeriaPolis ,NumberPolis ,BirthDay ,
          ...
          Age ,VZST ,K_KSG ,KSG_PG ,PVT ,IsDisableCheck,IT_SL, ENP, TypeCases, Quantity, TotalPriceMU, UR_K) 
...
--------------------------------------------------------------------------------------------------------------------------
-------------------DKK1-------------------
UPDATE s SET s.DKK1=a.rf_idAddCretiria
FROM dbo.t_SendingDataIntoFFOMS s INNER JOIN dbo.vw_sprCSG cs ON
			s.MES=cs.code			
								INNER JOIN dbo.t_AdditionalCriterion a ON
			s.rf_idCase=a.rf_idCase	
WHERE s.IsUnload=0 AND ReportYear>2017
-------------------DKK2-------------------
UPDATE s SET s.DKK2=a.rf_idV024
FROM dbo.t_SendingDataIntoFFOMS s INNER JOIN dbo.vw_sprCSG cs ON
			s.MES=cs.code			
								INNER JOIN dbo.t_CombinationOfSchema a ON
			s.rf_idCase=a.rf_idCase	
WHERE s.IsUnload=0 AND ReportYear>2017
...
WHERE MUSurgery IN ('B03.001.002','B03.001.003','B03.003.005','A16.04.064','A11.20.025.001') AND ReportYear>2017
