	DECLARE @reportYear SMALLINT=2018,
		@dateStart DATETIME='20180101',--всегда с начало года берутся случаи
		@dateEnd DATE=GETDATE(),		
		@dateStartPay DATETIME='20180101',--всегда с 10 числа отчетного месяца. В сентябре 2017 конечная дата изменилась, теперь нужно брать с 6 числа и по 10 число следующего месяца
...
--WHERE c.DateEnd>@dateStart AND c.DateEnd<@dateEnd AND c.id=81062412
...
SELECT DISTINCT c.id AS rf_idCase,c.rf_idMO AS CodeM,a.rf_idMO,a.ReportMonth,a.ReportYear,r.rf_idF008,c.rf_idV006,r.SeriaPolis,r.NumberPolis,p.BirthDay,p.rf_idV005
		...
		,c.IT_SL,ps.ENP,TypeCases,NULL AS quantity
...
SELECT DISTINCT c.id AS rf_idCase,c.rf_idMO AS CodeM,a.rf_idMO,a.ReportMonth,a.ReportYear,r.rf_idF008,c.rf_idV006,r.SeriaPolis,r.NumberPolis,p.BirthDay,p.rf_idV005
		...
		,'DIAL' AS K_KSG
		,0 AS KSG_PG
		,c.IT_SL,ps.ENP,TypeCases, m.Quantity
FROM UnitOfHosp u INNER JOIN dbo.t_Case c ON
		u.rf_idCase=c.id
					INNER JOIN dbo.t_RecordCasePatient r ON
		r.id=c.rf_idRecordCasePatient
