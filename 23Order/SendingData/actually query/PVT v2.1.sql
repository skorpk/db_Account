...
AS(
  ...
  FROM t_SendingDataIntoFFOMS2017 
  ...
  GROUP BY rf_idCase,ENP,DS1,DateBegin,DateEnd,rf_idV006
			   ...
			   FROM doubleDS d INNER JOIN (SELECT DISTINCT rf_idCase,ENP,DS1,DateBegin,DateEnd,rf_idV006 FROM  dbo.t_SendingDataIntoFFOMS2017 WHERE /*rf_idV006=@v6 AND*/ IsUnload=0) s ON	
					d.ENP=s.ENP
					AND d.DS1=s.DS1
...
FROM PVT3 d INNER JOIN dbo.t_SendingDataIntoFFOMS2017 s ON	
					d.ENP=s.ENP
					AND d.DS1=s.DS1
					AND d.dateBegin=s.DateBegin
					AND d.DateEnd=s.DateEnd
...
update dbo.t_SendingDataIntoFFOMS2017 SET PVT=0 WHERE PVT>0 and IsUnload=0 and IsFullDoubleDate=0 and IsDisableCheck=0

*/
--update dbo.t_SendingDataIntoFFOMS2017 SET IsDisableCheck=0 WHERE IsDisableCheck=1 
...
FROM dbo.t_SendingDataIntoFFOMS2017 s INNER JOIN (VALUES ('31',1),('32',1),('33',1),('84',1),('136',1),('137',1),('138',1),('139',1),('140',1),('141',1),('142',1),('143',1),('179',1),('295',1),
...
UPDATE dbo.t_SendingDataIntoFFOMS2017 SET IsDisableCheck=1 WHERE DateBegin=DateEnd AND rf_idV009=102 AND IsUnload=0
...
cteMin AS (SELECT TOP 1 WITH TIES d.ENP,d.DS1,DateBegin AS MinDateBegin,s.rf_idV006
			FROM doubleDS d INNER JOIN dbo.t_SendingDataIntoFFOMS2017 s ON	
					d.ENP=s.ENP
					AND d.DS1=s.DS1
					AND d.rf_idV006=s.rf_idV006
...
INTO #tmpDateBeg
FROM cteMin d INNER JOIN dbo.t_SendingDataIntoFFOMS2017 s ON	
					d.ENP=s.ENP
					AND d.DS1=s.DS1
...
from cteRepeat c1 inner JOIN cteRepeat c2 ON
		...
		AND c1.rf_idV006=c2.rf_idV006
				INNER JOIN dbo.t_SendingDataIntoFFOMS2017 s ON
...
FROM dbo.t_SendingDataIntoFFOMS2017 s INNER JOIN #tt t ON	
