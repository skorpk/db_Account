...
DECLARE @reportYear SMALLINT=2017,
		@dateStart DATETIME='20170101',--всегда с начало года берутся случаи
		@dateEnd DATE='20180101',		
		@dateStartPay DATETIME='20171209',--всегда с 10 числа отчетного месяца. В сентябре 2017 конечная дата изменилась, теперь нужно брать с 6 числа и по 10 число следующего месяца
...
FROM data_Send 
WHERE IsDelete=0 AND NOT EXISTS(SELECT * FROM dbo.t_SendingDataIntoFFOMS2017 WHERE rf_idCase=data_Send.rf_idCase)--включение случая происходит только один раз
--произвести расчет PVT
----передоривка символов DS2 и DS3
