USE AccountOMS
go
--необходимо заменить IdPeople на ENP
;WITH doubleDS
AS(
  SELECT rf_idCase,IdPeople,DS1,DateBegin, DateEnd,rf_idV006
  FROM t_SendingDataIntoFFOMS 
  WHERE PVT=0 /*AND rf_idV006=@v6	*/AND IsUnload=0
  GROUP BY rf_idCase,IdPeople,DS1,DateBegin,DateEnd,rf_idV006
),PVT3 as (SELECT s.IDPeople,s.DS1,d.dateBegin,d.DateEnd,d.rf_idV006
			   FROM doubleDS d INNER JOIN (SELECT DISTINCT rf_idCase,IdPeople,DS1,DateBegin,DateEnd,rf_idV006 FROM  dbo.t_SendingDataIntoFFOMS WHERE /*rf_idV006=@v6 AND*/ IsUnload=0) s ON	
					d.IDPeople=s.IDPeople
					AND d.DS1=s.DS1
					AND d.dateBegin=s.DateBegin		
					AND d.DateEnd=s.DateEnd
					AND d.rf_idV006=s.rf_idV006
				GROUP BY s.IDPeople,s.DS1,d.dateBegin,d.DateEnd,d.rf_idV006
				HAVING COUNT(*)>1)
...
FROM dbo.t_SendingDataIntoFFOMS s INNER JOIN (VALUES ('31',1),('32',1),('33',1),('84',1),('136',1),('137',1),('138',1),('139',1),('140',1),('141',1),('142',1),('143',1),('179',1),('295',1),
													...
													('45',2),('46',2),('50',2),('51',2),('52',2),('53',2),('54',2),('71',2),('108',2),('111',2)) v(csg,v6) ON
			              s.K_KSG=v.csg
...
;WITH doubleCase
AS(
	SELECT rf_idCase,IdPeople,DS1,rf_idV006 
	FROM t_SendingDataIntoFFOMS 
	WHERE PVT=0 /*AND rf_idV006=@v6*/ AND IsDisableCheck=0 AND IsFullDoubleDate=0 AND ReportYear=@reportYear
	GROUP BY rf_idCase,IdPeople,DS1,rf_idV006
),doubleDS as(SELECT IdPeople,DS1,rf_idV006 FROM doubleCase GROUP BY IdPeople,DS1,rf_idV006 HAVING COUNT(*)>1), 
cteMin AS (SELECT TOP 1 WITH TIES d.IDPeople,d.DS1,DateBegin AS MinDateBegin,s.rf_idV006
			FROM doubleDS d INNER JOIN dbo.t_SendingDataIntoFFOMS s ON	
					d.IDPeople=s.IDPeople
					AND d.DS1=s.DS1
					AND d.rf_idV006=s.rf_idV006
			WHERE /*s.rf_idV006=@v6 AND */s.IsDisableCheck=0 AND s.IsFullDoubleDate=0 AND ReportYear=@reportYear
			ORDER BY ROW_NUMBER() OVER(PARTITION BY d.IDPeople,d.DS1,s.rf_idV006 ORDER BY DateBegin,DateEnd)
			
			)
SELECT d.IDPeople, d.DS1,s.DateEnd,rf_idCase,s.rf_idV006
INTO #tmpDateBeg
...
;WITH cteRepeat
AS(
	SELECT distinct ROW_NUMBER() OVER(PARTITION BY d.IDPeople,d.DS1,d.rf_idV006 ORDER BY s.DateBegin,s.DateEnd) AS id,
			 s.rf_idCase, s.IDPeople,s.DS1,s.DateBegin,s.DateEnd,s.rf_idV006			 	
	FROM #tmpDateBeg d inner JOIN (SELECT DISTINCT IDPeople,DS1,DateBegin,DateEnd,rf_idCase,s.rf_idV006
								   from dbo.t_SendingDataIntoFFOMS s
								   WHERE s.IsFullDoubleDate=0 /*AND s.rf_idV006=@v6*/ AND ReportYear=@reportYear) s ON	
			d.IDPeople=s.IDPeople
			AND d.DS1=s.DS1	
			AND d.rf_idV006=s.rf_idV006
)
...
WHERE /*s.rf_idV006=@v6 AND */IsDisableCheck=0 AND s.IsFullDoubleDate=0	AND IsUnload=0	
