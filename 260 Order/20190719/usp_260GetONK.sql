USE AccountOMS
GO
CREATE PROCEDURE usp_260GetONK
as
DECLARE @dateStart DATETIME='20190101',	--всегда с начало года
		@dateEnd DATETIME=GETDATE(),
		@reportYear SMALLINT=2019,
		@reportMonth TINYINT,--отчетный мес€ц ставим сами т.к. случаи отбираем с начало года
		@fileName VARCHAR(26)='CT34_' --им€ файла мен€ем руками
...
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=@reportYear 
		...
		AND NOT EXISTS(SELECT 1 FROM dbo.t_260order_ONK WHERE rf_idCase=c.id) --AND c.id NOT IN(95663715,95663793,95713644) исключил данные случаи т.к. они €нварские и должны быть сн€ты из за не правильного оформлени€
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=d.rf_idCase
					INNER JOIN #tD dd ON
			d.DS1=dd.DiagnosisCode     					
					 INNER JOIN dbo.vw_DS_ONK dso ON
			c.id=dso.rf_idCase                   
					INNER JOIN dbo.t_PatientSMO ps ON
			r.id=ps.rf_idRecordCasePatient  					
...
FROM dbo.t_260order_ONK o INNER JOIN dbo.V_PaymentMethodRelation p ON
		o.IDSP=p.Code_old 
---по иногородним добавл€ю код —ћќ
