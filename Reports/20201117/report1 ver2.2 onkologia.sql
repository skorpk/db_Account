...
WHERE NOT EXISTS(SELECT 1 FROM PolicyRegister.dbo.PEOPLE p WHERE p.ENP=d.enp AND ISNULL(p.DS,'22220101')<@firstDayNextMonth) AND d.flag>27
UNION ALL
...
FROM #t t INNER JOIN dbo.t_PurposeOfVisit p on
		t.rf_idCase=p.rf_idCase
WHERE t.sid IS NOT NULL AND t.TypeFile='H' AND p.rf_idV025='1.3' AND t.USL_OK=3 AND p.DN =2 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP AND d.flag=t.flag) AND t.AmountPayment>0
UNION ALL
SELECT DISTINCT enp,flag,t.Age,t.Sex
FROM #t t INNER JOIN dbo.t_Case p on
		t.rf_idCase=p.id
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp=2 AND t.TypeDs=1 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP AND d.flag=t.flag) AND t.AmountPayment>0
UNION ALL
SELECT DISTINCT enp,t.flag,t.Age,t.Sex
FROM  dbo.t_DS2_Info p 	INNER JOIN oms_nsi.dbo.tmp_DS2report202007 d ON
             p.DiagnosisCode=d.КОД	
           ...
           p.rf_idCase=t.rf_idCase
		   AND d.flag=t.flag
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp=2 AND t.TypeDs=2 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP AND d.flag=t.flag) AND t.AmountPayment>0 AND d.flag>27
)
...
FROM dbo.DNPersons202007 d WHERE d.flag>27
UNION ALL
