...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			r.id=p.rf_idRecordCasePatient					
					left JOIN dbo.t_DispInfo dd ON
            c.id=dd.rf_idCase
WHERE f.DateRegistration>=@dateStartReg2 AND f.DateRegistration<@dateEndReg2  AND a.ReportYear=@reportYear2 AND c.rf_idV006=3 AND f.TypeFile='F' AND a.rf_idSMO<>'34' 
...
SELECT c.CodeM,l.NAMES AS LPU
		  ,CAST(SUM(CASE WHEN c.ReportMonth=11 AND c.Letter='U' THEN c.AmountPayment ELSE 0.0 END)/12 AS MONEY) AS U_Nov_Suum
		  ,CAST(sum(CASE WHEN c.ReportMonth=12 AND c.Letter='U' THEN c.AmountPayment ELSE 0.0 END) AS MONEY) AS U_Dec_Sum
		,count(CASE WHEN c.ReportMonth=11 AND c.Letter='U' THEN c.id ELSE null END)/12 AS U_Nov_Case
		,count(CASE WHEN c.ReportMonth=12 AND c.Letter='U' THEN c.id ELSE null END) AS U_Dec_Case
		-----------------------Disp D-----------------------------------
		  ,CAST(SUM(CASE WHEN c.ReportMonth=11 AND c.Letter='D' THEN c.AmountPayment ELSE 0.0 END)/12 AS MONEY) AS D_Nov_Suum
		  ,CAST(sum(CASE WHEN c.ReportMonth=12 AND c.Letter='D' THEN c.AmountPayment ELSE 0.0 END) AS MONEY) AS D_Dec_Sum
		,count(CASE WHEN c.ReportMonth=11 AND c.Letter='D' THEN c.id ELSE null END)/12 AS D_Nov_Case
		,count(CASE WHEN c.ReportMonth=12 AND c.Letter='D' THEN c.id ELSE null END) AS D_Dec_Case
		-----------------------Disp F-----------------------------------
		  ,CAST(SUM(CASE WHEN c.ReportMonth=11 AND c.Letter='F' THEN c.AmountPayment ELSE 0.0 END)/12 AS MONEY) AS F_Nov_Suum
		  ,CAST(sum(CASE WHEN c.ReportMonth=12 AND c.Letter='F' THEN c.AmountPayment ELSE 0.0 END) AS MONEY) AS F_Dec_Sum
		,count(CASE WHEN c.ReportMonth=11 AND c.Letter='F' THEN c.id ELSE null END)/12 AS F_Nov_Case
		,count(CASE WHEN c.ReportMonth=12 AND c.Letter='F' THEN c.id ELSE null END) AS F_Dec_Case
		-----------------------Disp R-----------------------------------
		  ,CAST(SUM(CASE WHEN c.ReportMonth=11 AND c.Letter='R' THEN c.AmountPayment ELSE 0.0 END)/12 AS MONEY) AS R_Nov_Suum
		  ,CAST(sum(CASE WHEN c.ReportMonth=12 AND c.Letter='R' THEN c.AmountPayment ELSE 0.0 END) AS MONEY) AS R_Dec_Sum
		,count(CASE WHEN c.ReportMonth=11 AND c.Letter='R' THEN c.id ELSE null END)/12 AS R_Nov_Case
		,count(CASE WHEN c.ReportMonth=12 AND c.Letter='R' THEN c.id ELSE null END) AS R_Dec_Case
		-----------------------Disp O-----------------------------------
		  ,CAST(SUM(CASE WHEN c.ReportMonth=11 AND c.Letter='O' and TypeDisp in ('ÄÂ1','ÄÂ3') THEN c.AmountPayment ELSE 0.0 END)/12 AS MONEY) AS R_Nov_Suum
		...
		,count(CASE WHEN c.ReportMonth=11 AND c.Letter='O' and TypeDisp in ('ÄÂ1','ÄÂ3') THEN c.id ELSE null END)/12 AS R_Nov_Case
		,count(CASE WHEN c.ReportMonth=12 AND c.Letter='O' and TypeDisp in ('ÄÂ1','ÄÂ3') THEN c.id ELSE null END) AS R_Dec_Case
		------------------------------------------------------------------------
		  ,CAST(SUM(CASE WHEN c.ReportMonth=11 AND c.Letter='O' and TypeDisp ='ÄÂ2' THEN c.AmountPayment ELSE 0.0 END)/12 AS MONEY)AS R_Nov_Suum
		...
		,count(CASE WHEN c.ReportMonth=11 AND c.Letter='O' and TypeDisp ='ÄÂ2' THEN c.id ELSE null END)/12 AS R_Nov_Case
