...
AND dd.TypeDiagnosis IN(1,3) AND f.TypeFile='H' 
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=d.rf_idCase									
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=2020 AND d.TypeDiagnosis IN(1,3) AND d.DiagnosisCode IN('U07.1','U07.2','B33.8','B34.2')
		and C.rf_idV009<316


SELECT COUNT(DISTINCT enp) AS AllSickPeople
--,COUNT(DISTINCT CASE WHEN rf_idV009 NOT IN(105,106,205,206,313,405,406,411) THEN enp ELSE NULL END) AS AllGoodPeople
,COUNT(DISTINCT enp)-COUNT(DISTINCT CASE WHEN rf_idV009 IN(105,106,205,206,313,405,406,411) THEN enp ELSE NULL END) AS AllGoodPeople
,COUNT(DISTINCT CASE WHEN rf_idV009 IN(105,106,205,206,313,405,406,411) THEN enp ELSE NULL END) AS AllDiedPeople
FROM #tECovid 
