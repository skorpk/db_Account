...
WHERE f.DateRegistration>=@dateStart AND f.DateRegistration<@dateEnd  AND a.ReportYear=@reportYear AND a.rf_idSMO<>'34' AND a.Letter='R'
...
FROM cteDouble c INNER JOIN #tCases cc ON
		...
		cc.CodeM=l.CodeM				
WHERE c.IdRow>1 AND cc.AmountPayment>0
ORDER BY ENP,LPU
GO
DROP TABLE #tCases