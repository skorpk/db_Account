...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			r.id=c.rf_idRecordCasePatient	
					INNER JOIN dbo.t_CompletedCase cc ON
			r.id=cc.rf_idRecordCasePatient	
			AND cc.DateEnd BETWEEN '20200301' AND '20200415'									
					INNER JOIN dbo.t_Diagnosis d ON
			c.id=d.rf_idCase					
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=2020 AND a.ReportMonth>2 AND a.ReportMonth<7 AND c.rf_idV006=1
...
---------------------------------------------Амбулаторная помощь------------------------------------
INSERT #tCases
SELECT DISTINCT c.id AS rf_idCase, f.CodeM, c.AmountPayment,3 AS TypeRequest, CAST(0.0 AS decimal(15,2)) AS AmountDeduction,c.rf_idRecordCasePatient,c.AmountPayment AS AmmPay
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
					INNER JOIN dbo.t_RecordCasePatient r ON
			a.id=r.rf_idRegistersAccounts
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			a.id=r.rf_idRegistersAccounts
					INNER JOIN dbo.t_Case c ON
			r.id=c.rf_idRecordCasePatient						
			AND c.DateEnd BETWEEN '20200301' AND '20200415'									
					INNER JOIN dbo.t_Diagnosis d ON
			c.id=d.rf_idCase					
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=2020 AND a.ReportMonth>2 AND a.ReportMonth<7 AND c.rf_idV006=3
	AND d.TypeDiagnosis IN(1,3) AND d.DiagnosisCode='B34.2' AND a.rf_idSMO<>'34'
...
SELECT 1 AS ColName
		,cast(SUM(CASE WHEN TypeRequest=1 THEN amountDeduction ELSE 0 END) as money) AS ColStacionarCovid
		,cast(SUM(CASE WHEN TypeRequest=3 THEN amountDeduction ELSE 0 END) as money) AS ColAMPCovid
		,cast(SUM(CASE WHEN TypeRequest=4 THEN amountDeduction ELSE 0 END) as money) AS ColSMPCovid
FROM (SELECT DISTINCT rf_idRecordCasePatient,TypeRequest,AmountDeduction FROM #tCases WHERE AmountDeduction>0) t
UNION all
SELECT 2 AS ColName
		,cast(SUM(CASE WHEN TypeRequest=1 THEN AmountPayment ELSE 0 END) as money) AS ColStacionarCovid
		,cast(SUM(CASE WHEN TypeRequest=3 THEN AmountPayment ELSE 0 END) as money) AS ColAMPCovid
		,cast(SUM(CASE WHEN TypeRequest=4 THEN AmountPayment ELSE 0 END) as money) AS ColSMPCovid
