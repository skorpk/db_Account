...
UNION ALL
SELECT DISTINCT enp,flag,t.Age,t.Sex
FROM #t t INNER JOIN dbo.t_DS2_Info p on
		t.rf_idCase=p.rf_idCase
...
FROM #t t INNER JOIN dbo.t_PurposeOfVisit p on
		t.rf_idCase=p.rf_idCase
WHERE t.sid IS NOT NULL AND t.TypeFile='H' AND p.rf_idV025='1.3' AND t.USL_OK=3 AND p.DN =2 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP) AND t.AmountPayment>0
UNION ALL
SELECT DISTINCT enp,flag,t.Age,t.Sex
FROM #t t INNER JOIN dbo.t_Case p on
		t.rf_idCase=p.id
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp=2 AND t.TypeDs=1 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP) AND t.AmountPayment>0
UNION ALL
SELECT DISTINCT enp,flag,t.Age,t.Sex
FROM #t t INNER JOIN dbo.t_DS2_Info p on
		t.rf_idCase=p.rf_idCase
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp=2 AND t.TypeDs=2 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP) AND t.AmountPayment>0
)
...
SELECT flag,COUNT(DISTINCT enp) AS Col8
		...
		----------Женщины--------------------------------------------------
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 59 THEN c.ENP ELSE NULL end) AS Col11
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=59 THEN c.ENP ELSE NULL end) AS Col12
...
SELECT flag,COUNT(DISTINCT enp) AS Col8
		...
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 59 THEN c.ENP ELSE NULL end) AS Col11
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=59 THEN c.ENP ELSE NULL end) AS Col12
FROM (SELECT distinct flag,ENP,Age,sex FROM dbo.T_inform202007 ) c
GROUP BY flag
-------------------------------------28 and 38-----------------------------------
;WITH cte
AS
...
SELECT flag,COUNT(DISTINCT c.rf_idCase) AS Col8
		...
		------- ---Женщины--------------------------------------------------
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 59 THEN c.rf_idCase ELSE NULL end) AS Col11
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=59 THEN c.rf_idCase ELSE NULL end) AS Col12
		------- --------Люди-----------------------------
		...
		------- ---Женщины--------------------------------------------------
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 59 THEN c.ENP ELSE NULL end) AS Col41
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=59 THEN c.ENP ELSE NULL end) AS Col42
FROM cte c
GROUP BY flag
-------------------------------------33 and 43-----------------------------------
;WITH cte
...
SELECT flag,COUNT(DISTINCT rf_idCase) AS Col8
		...
		------- ---Женщины--------------------------------------------------
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 59 THEN c.ENP ELSE NULL end) AS Col46
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=59 THEN c.ENP ELSE NULL end) AS Col47
FROM cte c
GROUP BY flag
...
FROM #t t INNER JOIN dbo.t_DS2_Info p on
		t.rf_idCase=p.rf_idCase
WHERE t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp IN(1,2) AND t.TypeDs=2 AND t.AmountPayment>0
)
...
SELECT flag,COUNT(DISTINCT c.rf_idRecordCasePatient ) AS Col8
		...
		------- ---Женщины--------------------------------------------------
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 59 THEN c.rf_idRecordCasePatient ELSE NULL end) AS Col11
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=59 THEN c.rf_idRecordCasePatient ELSE NULL end) AS Col12
