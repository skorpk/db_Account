...
-------------------------------------3----------------
/*
Расчет для 8 граффы будеткак для колонок 3-7
*/
;WITH cteCol4
AS(
...
FROM  dbo.t_DS2_Info p 	INNER JOIN oms_nsi.dbo.tmp_DS2report202007 d ON
           ...
           p.rf_idCase=t.rf_idCase
		   AND d.flag=t.flag
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp=2 AND t.TypeDs=2 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP AND d.flag=t.flag) AND t.AmountPayment>0
)
...
SELECT flag,COUNT(DISTINCT enp) AS Col8
		...
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 60 THEN c.ENP ELSE NULL end) AS Col11
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=60 THEN c.ENP ELSE NULL end) AS Col12
FROM (SELECT distinct flag,ENP,Age,sex FROM dbo.T_inform202007  WHERE Sex IS null) c
GROUP BY flag
