...
FROM  dbo.t_DS2_Info p 	INNER JOIN oms_nsi.dbo.tmp_DS2report202007 d ON
           ...
           p.rf_idCase=t.rf_idCase
		   AND d.flag=t.flag
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp=2 AND t.TypeDs=2 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP AND d.flag=t.flag) AND t.AmountPayment>0 AND t.ReportYear=@reportYear
)
