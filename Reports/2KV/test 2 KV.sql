...
FROM #tCases c INNER JOIN dbo.t_MES m on 
		m.rf_idCase=c.rf_idCase 		
			INNER JOIN #tLPU l ON
		c.rf_idMO=l.mcod							
WHERE /*c.TypeCase=2  AND */c.rf_idSMO<>'34' AND m.mes LIKE '2.89.%' --берутся все случаи, не обращщаем внимания на диагноз
--------------------------Не подушевое----------------------------------
UNION ALL
SELECT  14 AS IdRow,'2.2.11' AS ColName,c.rf_idMO AS MCOD
	,0 AS Col1,CAST(NULL AS INT) AS Col2,0.0 AS Col4
...
FROM #tCases c INNER JOIN dbo.t_MES m on 
		m.rf_idCase=c.rf_idCase 				
WHERE/* c.TypeCase=2  AND*/c.rf_idSMO<>'34' AND m.mes LIKE '2.89.%' AND NOT EXISTS (SELECT 1 FROM #tLPU WHERE mcod=c.rf_idMO)--берутся все случаи, не обращщаем внимания на диагноз
UNION all
-----------------------------------------------------------------2019---------------------------------------------------
