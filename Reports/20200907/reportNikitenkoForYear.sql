...
SELECT DISTINCT c.id AS rf_idCase, cc.AmountPayment,p.ENP,a.ReportMonth,a.ReportYear,DATEDIFF(YEAR,pp.BirthDay,GETDATE()) AS Age
		,DATEADD(year,1,CAST(a.ReportYear AS CHAR(4))+'0101') AS dd
INTO #t
...
--INSERT #t(AmountPayment,ENP,ReportMonth,ReportYear,dd,rf_idCase, age) SELECT 100.0,t.ENP,13,year,DATEADD(year,1,CAST(t.year AS CHAR(4))+'0101') AS dd,1 ,30 FROM dbo.tmpDN_1920 t 
...
;WITH cteTotal
AS(
SELECT 13 AS ReportMonth
		,CASE WHEN t.ReportYear=2019 THEN t.enp ELSE NULL END AS Col3
		,null AS Col4
		,CASE WHEN t.ReportYear=2020 THEN t.enp ELSE NULL END AS Col5
		,null AS Col6
FROM #t t
WHERE AmountPayment>0 AND t.sid IS NOT NULL
UNION all
SELECT 13 as ReportMonth,null
		,CASE WHEN t.ReportYear=2019 THEN t.enp ELSE NULL END AS Col4
		,0
		,CASE WHEN t.ReportYear=2020 THEN t.enp ELSE NULL END AS Col6
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
					INNER JOIN dbo.t_RecordCasePatient r ON
			a.id=r.rf_idRegistersAccounts
...
SELECT c.ReportMonth,COUNT(DISTINCT c.Col3) AS Col3,count(DISTINCT c.Col4) AS Col4,count(DISTINCT c.Col5) AS Col5,count(DISTINCT c.Col6) AS Col6
FROM cteTotal c 
