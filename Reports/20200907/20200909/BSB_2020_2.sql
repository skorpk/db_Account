...
SELECT rf_idCase, AmountPayment, ENP, @month AS ReportMonth, ReportYear, @dd AS dd,PID INTO #t  FROM dbo.tmpDSB2020 WHERE ReportYear=@year AND ReportMonth=@month AND pid IS NOT null
UNION all
SELECT 1, 0.0, ENP, @month, @year ,@dd ,pid FROM dbo.DNPersons1920_NEW WHERE YEAR=@year AND pid IS NOT null
