...
--DROP TABLE tmpDSB2020

CREATE UNIQUE NONCLUSTERED INDEX ix_1 ON #tDiag(DiagnosisCode)

SELECT DISTINCT c.id AS rf_idCase, cc.AmountPayment,p.ENP,a.ReportMonth,a.ReportYear,DATEDIFF(YEAR,pp.BirthDay,c.DateEnd) AS Age
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=dd.rf_idCase						
             ...
             c.id=pv.rf_idCase
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@year AND f.TypeFile='H' AND c.rf_idV006 =3 AND pv.DN IN (1,2) AND c.Age>16 AND a.rf_idSMO<>'34' AND a.ReportMonth<8 AND dd.TypeDiagnosis=1
...
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@year AND f.TypeFile='F'
	 AND c.rf_idV006 =3 AND c.IsNeedDisp IN(1,2) AND c.Age>16 AND a.rf_idSMO<>'34' AND a.ReportMonth<8
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=dd.rf_idCase						
             ...
             dd.DiagnosisCode=d.DiagnosisCode	
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@year AND f.TypeFile='F'
	 AND c.rf_idV006 =3 AND dd.IsNeedDisp IN(1,2) AND c.Age>16 AND a.rf_idSMO<>'34' AND a.ReportMonth<8
...
SELECT * INTO tmpDSB2020 FROM #t WHERE AmountPayment>0
GO
DROP TABLE #t
go
DROP TABLE #tDiag
