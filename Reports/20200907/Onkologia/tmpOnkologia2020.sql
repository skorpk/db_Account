...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=dd.rf_idCase						
             ...
             c.id=pv.rf_idCase
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@year AND a.ReportMonth<@month AND f.TypeFile='H' AND c.rf_idV006 =3 AND pv.DN IN (1,2)  AND a.rf_idSMO<>'34'
...
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@year AND a.ReportMonth<@month AND f.TypeFile='F'
...
SELECT * INTO tmpOnkologia2020 FROM #t WHERE AmountPayment>0
GO
DROP TABLE #t
go
DROP TABLE #tDiag
