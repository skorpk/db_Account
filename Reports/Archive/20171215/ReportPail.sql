...
CREATE TABLE #tmpCases(CodeM varchar(6),rf_idCase bigint,AmountPayment decimal(15, 2),AmountPaymentAccepted decimal(15, 2)
						...
						MU VARCHAR(9), MUName VARCHAR(250), MEK TINYINT,MEE TINYINT, EKMP TINYINT)
...
INSERT #tmpCases( CodeM ,rf_idCase ,AmountPayment ,AmountPaymentAccepted, MU,MUName )	
SELECT distinct f.CodeM,c.id AS rf_idCase,c.AmountPayment,c.AmountPayment ,mm.MU,mm.MUName
...
from #tmpCases c INNER JOIN dbo.vw_sprT001 l ON
			c.CodeM=l.CodeM
GROUP BY c.CodeM,l.NAMES,c.MUName,c.ReasonMEK,c.ReasonMEE,c.ReasonEKMP
go
drop TABLE #tmpCases
drop TABLE #tMU