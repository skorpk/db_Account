...
CREATE TABLE #tmpCases(CodeM varchar(6),rf_idCase bigint,AmountPayment decimal(15, 2),AmountPaymentAccepted decimal(15, 2)
						...
						MU VARCHAR(9), MUName VARCHAR(250), MEK TINYINT,MEE TINYINT, EKMP TINYINT,DS DATETIME )
...
IF @reportYear=2016
BEGIN
INSERT #tmpCases( CodeM ,rf_idCase ,AmountPayment ,AmountPaymentAccepted, MU,MUName,DS)	
SELECT distinct f.CodeM,c.id AS rf_idCase,c.AmountPayment,c.AmountPayment ,mm.MU,mm.MUName,pp.DS
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			p.pid=pp.ID                  
WHERE f.DateRegistration>=@dtBegin AND f.DateRegistration<=@dtEndReg AND c.rf_idV006=@v6 AND a.rf_idSMO IN('34001','34002','34006','34007') AND a.ReportYear=2016 AND a.ReportMonth<=12	
...
----------------------------------------------------------------------------------------------
SELECT c.CodeM,l.NAMES,c.MUName,COUNT(c.rf_idcase) AS CountCases
		,COUNT(CASE WHEN c.ds Is NOT NULL THEN c.rf_idCase ELSE NULL END ) AS CountDeath		
