...
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportMonth>0 AND a.ReportMonth<=@endMonth AND a.ReportYear=@reportYear
		AND a.rf_idSMO<>'34' AND c.rf_idV006=2
---------------------------------------------------------------------------------------		
...
FROM #tPeople p INNER JOIN dbo.t_MES m ON
			...
			m.MES=spr.MU
---------------------------------------------------------------------------------------
UPDATE p SET p.surgery=m.MUSurgery, MU=m.MU
FROM #tPeople p INNER JOIN dbo.t_Meduslugi m ON
	p.rf_idCase=m.rf_idCase
WHERE m.MUSurgery IS NOT null

UPDATE p SET  p.MU=m.MU
FROM #tPeople p INNER JOIN dbo.t_Meduslugi m ON
	p.rf_idCase=m.rf_idCase
WHERE m.MUGroupCode=55 AND m.MUUnGroupCode=2 AND m.MUCode IN (31,39)
...
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p         
WHERE AmountPayment>=0 AND DSTotal='C50' AND p.TypeMes=2 AND LastCSG IN (111,112,113) 
SET @iter+=1
...
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p         
WHERE AmountPayment>=0 AND DSTotal='C50' AND p.TypeMes=2 AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL
SET @iter+=1
...
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p         
WHERE AmountPayment>=0 AND DSTotal='C34' AND p.TypeMes=2 AND LastCSG IN (111,112,113)
SET @iter+=1
----------------------------19-----------------------------
INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p         
WHERE AmountPayment>=0 AND DSTotal='C34' AND p.TypeMes=2 AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL
SET @iter+=1
----------------------------20-----------------------------
INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
...
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p         
WHERE AmountPayment>=0 AND DSTotal='C18' AND p.TypeMes=2 AND LastCSG IN (111,112,113)
SET @iter+=1
----------------------------23-----------------------------
INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p         
WHERE AmountPayment>=0 AND DSTotal='C18' AND p.TypeMes=2 AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL
SET @iter+=1
----------------------------24-----------------------------
INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
...
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p         
WHERE AmountPayment>=0 AND DSTotal='C16' AND p.TypeMes=2 AND LastCSG IN (111,112,113)
SET @iter+=1
----------------------------27-----------------------------
INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p         
WHERE AmountPayment>=0 AND DSTotal='C16' AND p.TypeMes=2 AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL
SET @iter+=1
----------------------------28-----------------------------
INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
...
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p         
WHERE AmountPayment>=0 AND DSTotal='C61' AND p.TypeMes=2 AND LastCSG IN (111,112,113)
SET @iter+=1
----------------------------31-----------------------------
INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p         
WHERE AmountPayment>=0 AND DSTotal='C61' AND p.TypeMes=2 AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL
SET @iter+=1
----------------------------32-----------------------------
INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
...
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal>='C00' AND DSTotal<='C15' AND LastCSG IN (111,112,113)

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal='C17' AND LastCSG IN (111,112,113)

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal>='C19' AND DSTotal<='C33' AND LastCSG IN (111,112,113)

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal>='C35' AND DSTotal<='C49' AND LastCSG IN (111,112,113)

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal>='C51' AND DSTotal<='C60' AND LastCSG IN (111,112,113)

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal>='C62' AND DSTotal<='C80' AND LastCSG IN (111,112,113)

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal='C97' AND LastCSG IN (111,112,113)
SET @iter+=1
----------------------------35-----------------------------
INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal>='C00' AND DSTotal<='C15' AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal='C17' AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal>='C19' AND DSTotal<='C33' AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal>='C35' AND DSTotal<='C49' AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal>='C51' AND DSTotal<='C60' AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal>='C62' AND DSTotal<='C80' AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL

INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
from #tPeople p           
WHERE AmountPayment>=0 AND DSTotal='C97' AND LastCSG IN ( 107,108,109, 110) AND MU IS NOT NULL
SET @iter+=1
----------------------------36-----------------------------
INSERT @Total
SELECT @iter,COUNT(rf_idCase), SUM(isnull(AmountPayment,0))
