...
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=2016 AND a.ReportMonth<4 AND c.rf_idV006 IN(1,2) 
...
INTO t_Report2FFOMS
FROM #tPeople p	        
WHERE p.AmountDeduction>0 AND EXISTS(SELECT * FROM dbo.t_SendingDataIntoFFOMS f WHERE f.ReportYear=2016 AND r.rf_idCase=f.rf_idCase)	
