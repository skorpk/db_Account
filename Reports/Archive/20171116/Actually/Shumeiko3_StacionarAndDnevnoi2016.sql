USE AccountOMS
GO		
DECLARE @dtBegin DATETIME='20161001',	
		@dtEndReg DATETIME='20161231 23:59:59',
		@dtEndRegAkt DATETIME='20171122 23:59:59',
		@v6 TINYINT=2,--мен€ем услови€ оказани€ мед.помощи
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=p.rf_idCase                  					
					INNER JOIN PolicyRegister.dbo.PEOPLE ps	ON
			p.PID=ps.ID                  
					INNER JOIN dbo.vw_Diagnosis d ON
			c.id=d.rf_idCase  											
WHERE f.DateRegistration>=@dtBegin AND f.DateRegistration<=@dtEndReg AND a.rf_idSMO=@codeSMO AND c.rf_idV006=@v6
		AND NOT EXISTS(SELECT 1 
				   FROM dbo.t_MES m INNER JOIN dbo.vw_sprMUAndCSGUnit csg  ON
				   				m.MES=csg.MU                                                   
				   WHERE m.rf_idCase=c.id AND unitCode IN(141,152))
-----------------------------------------------------------------------------------------------
...
--WHERE DATEDIFF(DAY,t1.DateEnd,t2.DateBegin)>=0 AND DATEDIFF(DAY,t1.DateEnd,t2.DateBegin)<@dayDiff
)
...
WHERE diffD<@dayDiff
--CREATE NONCLUSTERED INDEX IX_Case_2 ON #tmpCases ([DS1],[rf_idV006],[ENP],[rf_idV002])
