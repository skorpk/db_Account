...
--as
--(																								 
--SELECT  DISTINCT t1.CodeM ,
--        t1.rf_idCase ,																			 
--        t1.AmountPayment ,
--        t1.AmountPaymentAccepted ,																 
--        t1.CodeSMO ,
--        t1.DS1 ,
--        t1.rf_idV006 ,
--        t1.ENP ,
...
--		t1.Account,
--		t1.DateAccount,
--		t1.NumberHistoryCase,
--		t1.idRecordCase,  
--		t1.NewBorn,      
--		ABS(DATEDIFF(DAY,t1.DateEnd,t2.DateBegin)) AS DayDiff
--FROM #tmpCases1 t1 INNER JOIN #tmpCases1 t2 ON
--		t1.ENP=t2.ENP
--		AND t1.DS1=t2.ds1
--		AND t1.rf_idV002 = t2.rf_idV002
--		AND t1.rf_idV006 = t2.rf_idV006
--		AND t1.NewBorn = t2.NewBorn
--		AND t1.rf_idCase <> t2.rf_idCase
--)
--SELECT * 
--INTO #tmpCases
--FROM cteD WHERE DayDiff<16 ORDER BY ENP,rf_idCase

;WITH cteD																						 
as
(																								 
SELECT  ROW_NUMBER() OVER(PARTITION BY ENP,DS1, rf_idV002,rf_idV006,NewBorn ORDER BY t1.rf_idCase ) AS id,
		t1.CodeM , t1.rf_idCase ,t1.AmountPayment ,t1.AmountPaymentAccepted ,t1.CodeSMO ,t1.DS1 ,t1.rf_idV006 ,t1.ENP ,t1.rf_idV002 ,t1.DateBegin ,t1.DateEnd ,t1.Account,	t1.DateAccount,	t1.NumberHistoryCase,	t1.idRecordCase,  	t1.NewBorn		
FROM #tmpCases1 t1
), cteA AS(
SELECT t1.id,t1.CodeM ,t1.rf_idCase ,t1.AmountPayment ,t1.AmountPaymentAccepted ,t1.CodeSMO ,
...
--SELECT * FROM #tmpCases WHERE ENP='0290799740000084'
...
SELECT distinct c.rf_idCase,c.ENP,c.CodeM,l.NAMES,c.DateBegin,c.DateEnd,c.DS1,c.Account,c.DateAccount,c.idRecordCase,CAST(c.AmountPayment AS MONEY) AS AmountPayment,CAST(c.AmountPaymentAccepted AS money) AS AmountPaymentAccepted,
		c.DateAkt,CASE WHEN c.TypeCheckup=2 THEN 'ÌÝÝ' WHEN c.TypeCheckup=3 then 'ÝÊÌÏ' ELSE '' END AS Typechekup,CAST(ISNULL(c.AmountDeduction,0.0) AS MONEY) AS Deduction,c.reason,c.NewBorn
