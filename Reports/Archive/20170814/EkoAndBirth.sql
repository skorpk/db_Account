...
DECLARE @dtBegin DATETIME='20160101',	
		...
		@dtBeginBirth DATETIME='20170101',	
		@dtEndBirth DATETIME='20170501',
		@dtEnd DATE='20161231',
...
WHERE c.ENP IS null
/*
SELECT COUNT(DISTINCT ENP) AS TotalENP, COUNT(DISTINCT rf_idCase) AS TotalCase FROM #tmpEKO WHERE AmountPayment>0

SELECT *  FROM #tmpEKO WHERE AmountPayment>0 AND ENP IS NULL
 */

SELECT f.codeM,c.id AS rf_idCase,c.AmountPayment,d.MUSurgery,c.DateBegin,c.DateEnd,ce.pid,ce.ENP,e.rf_idCase AS rf_idCaseEKO, c.Comments
INTO #tmpCases
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
					INNER JOIN dbo.t_RecordCasePatient r ON
			...
			c.id=d.rf_idCase
					INNER JOIN (VALUES('A16.20.005'),('B02.001.002'),('B01.001.006'),('B01.001.006.001'),('B01.001.009'),('B01.001.009.001') )v(MUSurgery) ON
			d.MUSurgery=v.MUSurgery
					INNER JOIN dbo.t_Case_PID_ENP ce ON
			c.id=ce.rf_idCase  
					INNER JOIN #tmpEKO e ON
			ce.ENP=e.ENP															                  
WHERE f.DateRegistration>=@dtBeginBirth AND f.DateRegistration<@dtEndBirth AND a.ReportYearMonth<=201705
...
from #tmpCases c INNER JOIN ( SELECT c.rf_idCase,SUM(c.AmountDeduction) AS AmountDeduction
								FROM ExchangeFinancing.dbo.t_AFileIn f INNER JOIN  ExchangeFinancing.dbo.t_DocumentOfCheckup d ON
														...
														c.rf_idCase=cc.rf_idCase																							
								WHERE f.DateRegistration>=@dtBeginBirth AND f.DateRegistration<@dtEndBirth
								GROUP BY c.rf_idCase
							) p ON
			c.rf_idCase=p.rf_idCase 

SELECT c.CodeM, l.NameS,count(DISTINCT PID)
