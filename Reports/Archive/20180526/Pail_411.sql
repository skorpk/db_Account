...
WHERE f.DateRegistration>=@dtStart AND f.DateRegistration<@dtEnd AND a.ReportYear=@reportYear AND a.ReportMonth>=1 AND a.ReportMonth<5 AND c.rf_idV006=3
		AND c.DateEnd>='20180101' AND c.DateEnd<'20180501' AND a.rf_idSMO<>'34'
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
		...
		r.id=ps.rf_idRecordCasePatient			
				INNER JOIN dbo.t_Case c  ON
		r.id=c.rf_idRecordCasePatient							
			INNER JOIN (VALUES(411)) v(rf_idV009) ON--RSLT=411 не брать
			c.rf_idV009=v.rf_idV009 
...
WHERE f.DateRegistration>=@dtStart AND f.DateRegistration<@dtEnd AND a.ReportYear=@reportYear AND a.ReportMonth>=1 AND a.ReportMonth<5
		AND c.DateEnd>='20180101' AND c.DateEnd<'20180501' AND a.rf_idSMO<>'34' AND NOT EXISTS(SELECT * FROM #tmpENP WHERE ENP=ps.ENP)
...
FROM dbo.t_PaymentAcceptedCase2 c INNER JOIN #tmpPeople p ON
			...
			AND c.idAkt=r.idAkt                  
WHERE c.DateRegistration>=@dtStart AND c.DateRegistration<@dtEnd And c.TypeCheckup=3
