...
DECLARE @startDateReg DATETIME='20180110',
		@endDateReg DATETIME='20181015',
		@endDateRegAkt DATETIME='20181015',
		@reportYear smallint=2018,
		@reportMonth TINYINT=9,
		@codeSMO CHAR(5)='34002',
		@dtEndB DATE='20181001'

--SET STATISTICS TIME, IO ON
SELECT c.id AS rf_idCase,c.AmountPayment,f.CodeM,r.ENP, DATEDIFF(YEAR,rp.BirthDay,GETDATE()) AS Age,rp.rf_idV005, CAST(0 AS DECIMAL(11,2)) AS AmountPaymentAccepted, CAST(0 AS DECIMAL(11,2)) AS AmountPay
...
WHERE f.DateRegistration>@startDateReg AND f.DateRegistration<=@endDateReg AND a.ReportMonth<=@reportMonth AND a.ReportYear=@reportYear 
		AND c.DateEnd>='20180101' AND c.DateEnd<@dtEndB AND a.Letter='O' AND d.TypeDisp IN ('ÄÂ1','ÄÂ2','ÄÂ3')
...
ON #tPeople ([rf_idV005],[Age],[TypeDisp])
INCLUDE ([rf_idCase],[ENP],[AmountPaymentAccepted],[AmountPay],[IsMobileTeam],[RSLT],[IsNeedDisp],[IsCanser],[IsLastCase],[IsTypeDisp],[IsCalc28])

CREATE NONCLUSTERED INDEX ix_tmp2
ON #tPeople ([rf_idCase]) include([ENP],[AmountPaymentAccepted],[AmountPay],[IsMobileTeam],[RSLT],[IsNeedDisp],[IsCanser],[IsLastCase],[IsTypeDisp],[IsCalc28], TypeDisp,Age, rf_idV005)
...
CREATE NONCLUSTERED INDEX IX_Prescription ON #prescription ([rf_idCase]) INCLUDE ([NAZR],[rf_idV015])
...
FROM #tPeople p INNER JOIN (
							...
							FROM dbo.t_PaymentAccepted p INNER  JOIN #tPeople t ON			
...
----óäàëÿåì çàïèñè êîòîðûå íå âîçìîæíî îïðåäåëèòü ê êàêîé òî îïðåäåëåííîé ãðóïïå
DELETE FROM #tPeople WHERE TypeDisp='ÄÂ2' AND IsTypeDisp IS null
...
WHERE id=1
-- íå áûëî ïðîâåðêè
delete from #tPeople where TypeDisp in ('ÄÂ2') and IsTypeDisp is null
...
SELECT 1,0 AS Col3,0 AS Col4,0 AS Col5,0 AS Col6,0 AS Col7
	 ...
	 ----------------------------------------------------------------------------------------------------------------------	  
	 ,COUNT( DISTINCT CASE WHEN (AmountPaymentAccepted>0 AND AmountPay>0 AND IsLastCase=1  AND TypeDisp='ÄÂ2') OR (AmountPaymentAccepted>0 AND IsCalc28=1 AND AmountPay>0 AND TypeDisp IN('ÄÂ1','ÄÂ3') AND IsLastCase=1) THEN ENP ELSE NULL END) AS col28
	 ,COUNT( DISTINCT CASE WHEN  (AmountPaymentAccepted>0 AND IsCalc28=1 AND AmountPay>0 AND TypeDisp IN('ÄÂ1','ÄÂ3') AND IsLastCase=1) THEN ENP ELSE NULL END) AS col28
