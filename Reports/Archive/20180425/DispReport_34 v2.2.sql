...
SELECT 1,0 AS Col3,0 AS Col4,0 AS Col5,0 AS Col6,0 AS Col7
	 ...
	 ----------------------------------------------------------------------------------------------------------------------	  
	 ,COUNT( DISTINCT CASE WHEN (AmountPaymentAccepted>0 AND AmountPay>0 AND TypeDisp='ÄÂ2') OR (AmountPaymentAccepted>0 AND IsCalc28=1 and AmountPay>0 AND TypeDisp IN('ÄÂ1','ÄÂ3') AND IsLastCase=1) THEN ENP ELSE NULL END) AS col28
	 ,COUNT( DISTINCT CASE WHEN (AmountPaymentAccepted>0 AND IsCalc28=1 AND AmountPay>0 AND TypeDisp IN('ÄÂ1','ÄÂ3') AND IsLastCase=1 AND TypeFailure=1) THEN ENP ELSE NULL END) AS col29
