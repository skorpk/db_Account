...
FROM #tPeople p LEFT JOIN #prescription pp ON
		p.rf_idCase=pp.rf_idCase
WHERE rf_idV005=1 AND TypeDisp IN('ÄÂ3','ÄÂ2') AND ISNULL(IsTypeDisp,2)=2 
UNION ALL
----------------------------------------------------------Women-------------------------------------------
...
FROM #tPeople p LEFT JOIN #prescription pp ON
		p.rf_idCase=pp.rf_idCase
WHERE rf_idV005=2 AND TypeDisp IN('ÄÂ3','ÄÂ2') AND ISNULL(IsTypeDisp,2)=2
UNION ALL
...
SELECT 1,COUNT(DISTINCT ENP2) AS Col5
		...
		,COUNT(DISTINCT CASE WHEN e.Date_B<'20180401' then ENp2 else NULL END) AS Col7
		...
		,count(DISTINCT CASE WHEN p.AmountPaymentAccepted>0 AND p.TypeDisp IN('ÄÂ1','ÄÂ3') AND p.AmountPay>0 AND stage=1 AND Date_I<'20180401' THEN ENP3 ELSE null END) AS Col120
		,count(DISTINCT CASE WHEN p.AmountPaymentAccepted>0 AND p.TypeDisp ='ÄÂ2' AND p.AmountPay>0 AND stage=2 AND Date_I<'20180401' THEN ENP3 ELSE null END) AS Col124
