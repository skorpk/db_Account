...
------------------------------- 2-пациенто-дни----------------------------------
INSERT #tmp( CodeM ,CodeSMO ,UnitAccounting ,MonthQuantity ,MonthPeople ,MonthAmount ,YearQuantity ,YearPeople ,YearAmount)
SELECT t.CodeM,t.rf_idSMO,UnitAccounting,SUM(MonthQuantity),COUNT(MonthPeople) ,SUM(MonthAmount),SUM(YearQuantity),COUNT(YearPeople),SUM(YearAmount)
FROM (
...
----------------------------- 4-вызов СМП--------------------------------
INSERT #tmp( CodeM ,CodeSMO ,UnitAccounting ,MonthQuantity ,MonthPeople ,MonthAmount ,YearQuantity ,YearPeople ,YearAmount)
SELECT t.CodeM,t.rf_idSMO,t.UnitAccounting,SUM(t.Quantity),COUNT(MonthPeople),SUM(AmountPayment),SUM(YearQuantity),COUNT(YearPeople),SUM(YearAmount)
FROM (
		SELECT c.id,f.CodeM,a.rf_idSMO,4 AS UnitAccounting,SUM(m.Quantity) AS Quantity ,p.IDPeople as MonthPeople,c.AmountPayment,0 AS YearQuantity,null AS YearPeople,0 AS YearAmount
		FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
					f.id=a.rf_idFiles
					AND a.ReportMonth=@reportMonth
					AND a.ReportYear=@reportYear
...
----------------------------- 5-врачебные приемы------------------------------
INSERT #tmp( CodeM ,CodeSMO ,UnitAccounting ,MonthQuantity ,MonthPeople ,MonthAmount ,YearQuantity ,YearPeople ,YearAmount)
SELECT t.CodeM,t.rf_idSMO,t.UnitAccounting,SUM(t.MonthQuantity),COUNT(MonthPeople),SUM(MonthAmount),SUM(YearQuantity),COUNT(YearPeople),SUM(YearAmount) 
from (
