...
FROM (
	...
	FROM #tPeople p INNER JOIN dbo.t_DS2_Info r ON
			...
			p.PID=pp.PID
	WHERE pp.rf_idV014<3 AND p.AmountPayment>0 AND r.IsNeedDisp IN(1,2) AND p.Letter IN('D','F','V','I','U') AND pp.AmountPayment>0
	UNION ALL
	-------Col3-----------
	...
	FROM #tPeople p INNER JOIN dbo.t_DS2_Info r ON
			...
			p.PID=pp.PID
				...
				m.MU=mm.MU 
	WHERE p.AmountPayment>0 AND pp.AmountPayment>0 AND r.IsNeedDisp IN(1,2) AND pp.rf_idV006=3 AND p.Letter IN('D','F','V','I','U') AND pp.CodeM=pp.AttachLPU
	UNION ALL
	
	-------Col4-----------
	SELECT 0,0 ,0,COUNT(DISTINCT PID) AS Col4,0,0,0
	FROM #tPeople p 
	WHERE rf_idV014<3 AND AmountPayment>0 AND Letter IN('D','F','V','I','U') AND NOT EXISTS(SELECT 1 FROM dbo.t_DS2_Info WHERE rf_idCase=p.rf_idCase AND IsNeedDisp=1)
	UNION ALL
	SELECT 0,0 ,0,COUNT(DISTINCT PID) AS Col4,0,0,0
	FROM #tPeople p 
	...
	FROM #tPeople p INNER JOIN dbo.t_DS2_Info r ON
			p.rf_idCase=r.rf_idCase
	WHERE p.AmountPayment>0 AND p.Letter IN ('D','F','V','I','U') AND r.IsNeedDisp>0
