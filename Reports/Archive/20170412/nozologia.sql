...
DECLARE @dtStart DATETIME='20170101',  --дата начало регистрации
		@dtEnd DATETIME='20170401 23:59:59',--дата окончания регистрации
		@dtEnd DATETIME='20170411',--дата окончания регистрации РАК
		@reportMM TINYINT=3,
...
FROM #tPeople p INNER JOIN (SELECT rf_idCase,SUM(AmountDeduction) AS AmountDeduction 
							...
							WHERE DateRegistration>=@dtStart AND DateRegistration<=@dtEndRAK
							GROUP BY rf_idCase
							) r ON
			p.rf_idCase=r.rf_idCase
																
			
SELECT d.groupName,CAST(SUM(p.AmountPayment) AS MONEY) AS AmountRAK
