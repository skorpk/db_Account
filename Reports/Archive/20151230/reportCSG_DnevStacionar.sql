...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=m.rf_idCase	
					INNER JOIN (SELECT  code,'Д' AS Duration FROM dbo.vw_sprCSG WHERE SUBSTRING(code,3,1) IN('0','3','4','5','6')
								UNION ALL
								SELECT  code,'К' AS Duration FROM dbo.vw_sprCSG WHERE SUBSTRING(code,4,1)='3'
								)	csg ON
			m.MES=csg.code                  
...
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.Surgery=c.Sur
WHERE c.Step=2 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111 AND c.rf_idV006=2			
-------------Step 1---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.DS=c.DS1
WHERE c.Step=1 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111 AND c.rf_idV006=2
...
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.DS=c.DS1		
		AND p.Surgery=c.Sur
WHERE c.Step=3 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111 AND c.rf_idV006=2
-------------Step 4---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		...
		AND p.Surgery=c.Sur		
		AND p.Age=c.Age
WHERE c.Step=4 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111 AND c.rf_idV006=2
-------------Step 7---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.DS=c.DS1		
		AND p.Age=c.age
WHERE c.Step=7 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111 AND c.rf_idV006=2		
-------------Step 8---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.DS=c.DS1		
		AND p.Sex=c.Sex
WHERE c.Step=8   AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111 AND c.rf_idV006=2
-------------Step 9---------------
--INSERT #tCaseCSG( rf_idCase, idCSG )
--SELECT DISTINCT p.rf_idCase,c.id
--FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
...
--WHERE c.Step=9 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111 AND c.rf_idV006=2
-------------Step 10 ---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.Age=c.Age
		AND p.Surgery=c.Sur
 WHERE c.Step=10	AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111 AND c.rf_idV006=2 
-------------Step 11---------------
/*
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.DS=c.DS1
		AND p.DS2=c.DS2
WHERE c.Step=11 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111	AND c.rf_idV006=2
...
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.Surgery=c.Sur		
		AND p.Sex=c.Sex
WHERE c.Step=12 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111 AND c.rf_idV006=2		
-------------Step 13--------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.Surgery=c.Sur		
		AND p.Duration=c.LOS	
WHERE c.Step=13 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111 AND c.rf_idV006=2
-------------Step 14--------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		...
		AND p.Surgery=c.Sur		
		AND p.Age=c.age
WHERE c.Step=14 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<111	AND c.rf_idV006=2		
*/
INSERT #tCaseCSG( rf_idCase, idCSG )
