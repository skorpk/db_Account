...
INSERT #tPeople( rf_idCase ,CodeM ,CodeSMO ,AmountPayment,Age,Duration,Sex,DurationCase)
SELECT c.id,f.CodeM,a.rf_idSMO,c.AmountPayment,5,CASE WHEN DATEDIFF(DAY,c.DateBegin,c.DateEnd)>2 THEN 1 ELSE NULL END,p.Sex,csg.Duration
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=m.rf_idCase	
					INNER JOIN (SELECT  code,CASE WHEN SUBSTRING(code,3,2) IN('00','06','07','08','09') THEN 'Д' ELSE 'К' END AS Duration
								FROM dbo.vw_sprCSG WHERE SUBSTRING(code,3,2) IN('00','03','04','06','07','08','09')
								)	csg ON
			m.MES=csg.code                  
...
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportMonth>0 AND a.ReportMonth<=12 AND a.ReportYear=2015 AND c.rf_idV006<>32		
		AND c.Age>18 AND c.rf_idV006=1
----------------разбивка детей по группам--------------------		
INSERT #tPeople( rf_idCase ,CodeM ,CodeSMO ,AmountPayment,Age,Duration,Sex,DurationCase)
SELECT c.id,f.CodeM,a.rf_idSMO,c.AmountPayment,v.ageGroup,CASE WHEN DATEDIFF(DAY,c.DateBegin,c.DateEnd)>2 THEN 1 ELSE NULL END,p.Sex,csg.Duration
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
			AND a.rf_idSMO<>'34'
					INNER JOIN dbo.t_RecordCasePatient r ON
...
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.Surgery=c.Sur
WHERE c.Step=2 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300 AND c.rf_idV006=1			
-------------Step 1---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.DS=c.DS1
WHERE c.Step=1 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300 AND c.rf_idV006=1
...
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.DS=c.DS1		
		AND p.Surgery=c.Sur
WHERE c.Step=3 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300 AND c.rf_idV006=1
-------------Step 4---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		...
		AND p.Surgery=c.Sur		
		AND p.Age=c.Age
WHERE c.Step=4 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300 AND c.rf_idV006=1
-------------Step 7---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.DS=c.DS1		
		AND p.Age=c.age
WHERE c.Step=7 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300 AND c.rf_idV006=1		
-------------Step 8---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.DS=c.DS1		
		AND p.Sex=c.Sex
WHERE c.Step=8   AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300 AND c.rf_idV006=1
-------------Step 9---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
...
WHERE c.Step=9 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300 AND c.rf_idV006=1
-------------Step 10 ---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.Age=c.Age
		AND p.Surgery=c.Sur
 WHERE c.Step=10	AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300 AND c.rf_idV006=1 
-------------Step 11---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.DS=c.DS1
		AND p.DS2=c.DS2
WHERE c.Step=11 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300	AND c.rf_idV006=1
/* Missing step
-------------Step 12---------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.Surgery=c.Sur		
		AND p.Sex=c.Sex
WHERE c.Step=12 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300 AND c.rf_idV006=1		
-------------Step 13--------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		p.Surgery=c.Sur		
		AND p.Duration=c.LOS	
WHERE c.Step=13 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300 AND c.rf_idV006=1
*/
-------------Step 14--------------
INSERT #tCaseCSG( rf_idCase, idCSG )
SELECT DISTINCT p.rf_idCase,c.id
FROM #tPeople p INNER JOIN dbo.tmp_CSG2016 c ON
		...
		AND p.Surgery=c.Sur		
		AND p.Age=c.age
WHERE c.Step=14 AND p.AmountPayment>0 AND c.CSG>0 AND c.CSG<300	AND c.rf_idV006=1		
