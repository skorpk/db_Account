USE AccountOMS
GO
declare @dateStartReg DATETIME='20170321',
		@dateEndReg DATETIME='20170606 23:59:59',
		@reportYear SMALLINT=2017,
		@reportMonthStart TINYINT=3,
...
FROM PeopleAttach.dbo.CancerRegistr p INNER JOIN dbo.t_Case_PID_ENP c ON
		p.ENP=c.ENP 
WHERE p.ENP IS NOT NULL and c.ReportYear=@reportYear AND p.[YEAR]=@reportYear AND p.[MONTH]>=@reportMonthStart AND p.[MONTH]<=@reportMonthEnd
