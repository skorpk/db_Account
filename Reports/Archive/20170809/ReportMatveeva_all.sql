...
SELECT p.MES,csg.name,p.Tariff,p.AmountPayment2, v2.name,COUNT(p.id),'Да' AS IsAccepted
...
UNION ALL
SELECT p.MES,csg.name,p.Tariff,p.AmountPayment2, v2.name,COUNT(p.id),'Нет'
FROM #tmpPeople p INNER JOIN RegisterCases.dbo.vw_sprCSG csg ON
		p.MES=csg.code
				INNER JOIN RegisterCases.dbo.vw_sprV002 v2 ON
		p.rf_idV002=v2.id              
