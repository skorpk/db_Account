...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=ps.rf_idCase			
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportMonth>=@mmBegin AND a.ReportMonth<=@mmEnd AND a.ReportYear=@reportYear AND c.rf_idV006 IN(1,3)
AND pid IS NOT NULL

; WITH cteAmb
AS(
--SELECT *
--FROM #tPeople p WHERE rf_idV006=3 AND EXISTS(SELECT 1 FROM #tPeople WHERE rf_idV006=1 AND P.DateBegin>DateBegin AND p.DateBegin<DateEnd AND pid=p.pid AND rf_idSMO=p.rf_idSMO)
SELECT *
FROM #tPeople p WHERE rf_idV006=3 AND EXISTS(SELECT 1 FROM #tPeople WHERE rf_idV006=1 AND P.DateBegin<DateEnd AND p.DateEnd>DateBegin AND pid=p.pid AND rf_idSMO=p.rf_idSMO)
--UNION 
--SELECT *
--FROM #tPeople p WHERE rf_idV006=3 AND EXISTS(SELECT 1 FROM #tPeople WHERE rf_idV006=1 AND P.DateEnd>DateBegin AND p.DateEnd<DateEnd AND pid=p.pid AND rf_idSMO=p.rf_idSMO)
--UNION --третий когда стационар полностью включен в амбулаторку
--SELECT *
--FROM #tPeople p WHERE rf_idV006=3 AND EXISTS(SELECT 1 FROM #tPeople WHERE rf_idV006=1 AND P.DateEnd>DateEnd AND p.DateBegin<DateBegin AND pid=p.pid AND rf_idSMO=p.rf_idSMO)
)
...
AS(
SELECT *
FROM #tPeople p WHERE rf_idV006=1 AND EXISTS(SELECT 1 FROM #tPeople WHERE rf_idV006=3 AND P.DateBegin<DateEnd AND p.DateEnd>DateBegin AND pid=p.pid AND rf_idSMO=p.rf_idSMO)
--SELECT *
--FROM #tPeople p WHERE rf_idV006=1 AND EXISTS(SELECT 1 FROM #tPeople WHERE rf_idV006=3 AND P.DateBegin>DateBegin AND p.DateBegin<DateEnd AND pid=p.pid AND rf_idSMO=p.rf_idSMO)
--UNION 
--SELECT *
--FROM #tPeople p WHERE rf_idV006=1 AND EXISTS(SELECT 1 FROM #tPeople WHERE rf_idV006=3 AND P.DateEnd>DateBegin AND p.DateEnd<DateEnd AND pid=p.pid AND rf_idSMO=p.rf_idSMO)
)
