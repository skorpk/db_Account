...
CREATE UNIQUE NONCLUSTERED INDEX QU_IdCase ON #tCases(rf_idCases) WITH IGNORE_DUP_KEY
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=d.rf_idCase																		
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND /*a.ReportMonth>8 AND */a.ReportMonth=11 AND a.ReportYear=@reportYear AND c.rf_idV006<>4
		AND (d.DS_ONK=1 OR dd.DS1 LIKE 'c%') 
INSERT #tCases( rf_idCases, CodeM,AmountPayment )
SELECT c.id, f.CodeM, c.AmountPayment
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
			...
			r.id=c.rf_idRecordCasePatient
					INNER JOIN dbo.vw_Diagnosis dd ON
			c.id=dd.rf_idCase	
					left JOIN dbo.t_DispInfo d ON
			c.id=d.rf_idCase																			
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND /*a.ReportMonth>8 AND */a.ReportMonth=11 AND a.ReportYear=@reportYear AND c.rf_idV006<>4
		AND (d.IsOnko=1 OR dd.DS1 LIKE 'c%')
...
FROM #tCases c inner JOIN dbo.t_PaymentAcceptedCase3 p ON
		c.rf_idCases=p.rf_idCase
				LEFT JOIN dbo.t_ReasonDenialPayment3 r ON
