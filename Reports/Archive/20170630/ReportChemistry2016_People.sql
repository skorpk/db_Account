...
FROM (
		...
		FROM dbo.vw_sprCSG WHERE code LIKE '____03[1-3]'
		UNION ALL
		SELECT 1,code,name
		FROM dbo.vw_sprCSG WHERE code LIKE '____13[6-9]'
		UNION ALL
		SELECT 1,code,name
		FROM dbo.vw_sprCSG WHERE code LIKE '____140'
		-----Дневной стационар---------------
		UNION ALL
		SELECT 2,code,name
		FROM dbo.vw_sprCSG WHERE code LIKE '____01[3-5]'
		UNION ALL
		SELECT 2,code,name
		FROM dbo.vw_sprCSG WHERE code LIKE '____05[0-4]'
	) t
				

SELECT f.CodeM,c.id AS rf_idCase, r.id AS rf_idRecordCasePatient, f.id AS rf_idFile,c.AmountPayment,c.rf_idV006,pid, DateBegin,DateEnd,MES,cs.name AS CSGName,pc.ENP
INTO #tmp
