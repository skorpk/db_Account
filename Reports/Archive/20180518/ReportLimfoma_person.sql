...
SELECT c.id,a.rf_idSMO, m.MES, c.AmountPayment, c.AmountPayment AS AmountPaymentAcc,c.rf_idV008, c.Age,
		c.DateBegin, c.DateEnd, d1.DS1, ISNULL(r.SeriaPolis,'')+r.NumberPolis AS NumberPolis, d1.TypeCol
INTO #tmpPeople
...
FROM #tmpPeople p INNER JOIN (SELECT c.rf_idCase,SUM(c.AmountDeduction) AS AmountDeduction
			...
			p.id=r.rf_idCase
-------------------------Adults---------------------------
SELECT DISTINCT NumberPolis,p.Age,p.DateBegin,p.DateEnd, v8.Name, p.MES, p.DS1,p.AmountPayment 
