...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
		...
		d.DS1=d1.DS1		              
WHERE f.DateRegistration>=@dtStart AND f.DateRegistration<@dtEnd AND a.ReportYear=@reportYear AND c.rf_idV006=1 AND a.rf_idSMO='34'
...
FROM #tmpPeople p INNER JOIN (SELECT c.rf_idCase,SUM(c.AmountDeduction) AS AmountDeduction
			...
			p.id=r.rf_idCase
-------------------------Adults---------------------------
SELECT TypeCol, n.Name,n.DS,COUNT(p.id) AS Col3,CAST(SUM(AmountPayment) AS MONEY) AS Col5, COUNT(CASE WHEN MES IN('10100143', '10200143', '10000146') THEN p.id ELSE NULL END) AS col7,
...
ORDER BY TypeCol
--------------------------children--------------------------------
SELECT TypeCol, n.Name,n.DS,COUNT(p.id) AS Col3,CAST(SUM(AmountPayment) AS MONEY) AS Col5, COUNT(CASE WHEN MES IN('10100032', '10200032') THEN p.id ELSE NULL END) AS col7,
		COUNT(CASE WHEN rf_idV008=32 THEN p.id ELSE NULL END) AS Col9, 
		CAST(sum(CASE WHEN rf_idV008=32 THEN AmountPayment ELSE 0 END)  AS MONEY) AS Col11
FROM #tmpPeople	p INNER JOIN #tName n ON
			p.TypeCol=n.id
