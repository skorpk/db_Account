...
where	B.code in (38,14,15,40) and 
		A.endDate >= '20150101' and 
		year(D.beginDate) <= 2015 and 
		yeaR(case when D.terminationDate < D.endDate then D.terminationDate else D.endDate end) >= 2015 --and 
order by 2,5,8

CREATE NONCLUSTERED INDEX ix_codem ON #tmpLPU(CodeM) 
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
		...
		f.CodeM=l.CodeM			
WHERE f.DateRegistration>=@dtStart AND f.DateRegistration<@dtEnd AND a.ReportYear=@Year AND a.ReportMonth<=@month AND a.rf_idSMO<>'34' 
...
WHERE c.DateEnd>= t1.beginDate AND c.DateEnd<=t1.endDate AND p.rf_idV006=3 AND p.Letter IN('K','T')
		AND p.AmountPayment>0
GROUP BY c.id,t1.unitName
UNION ALL
 SELECT c.id
		,'Посещения для счетов не K и T' AS unitName
		,SUM(m.Quantity) AS Quantity
FROM #tmpPeople p INNER JOIN t_Case c ON
		p.id=c.id
				INNER JOIN t_Meduslugi m ON
		c.id=m.rf_idCase 				
WHERE p.rf_idV006=3 AND p.Letter NOT IN ('K','T') AND m.MUGroupCode=2 AND p.AmountPayment>0
GROUP BY c.id
UNION ALL
SELECT c.id
		,t1.unitName
...
FROM #tmpPeople p INNER JOIN t_Case c ON
		...
		m.MES=t1.MU			
		AND t1.unitCode IS NOT NULL			
WHERE c.DateEnd>= t1.beginDate AND c.DateEnd<=t1.endDate AND p.rf_idV006=3 AND p.Letter IN('K','T')	AND p.AmountPayment>0
GROUP BY c.id,t1.unitName
UNION ALL
SELECT c.id
		,t1.unitName
