USE AccountOMS
GO
DECLARE @dateStart DATETIME='20160801',
		@dateEnd DATETIME='20170125',
		@dateEndPay DATETIME='20170801 23:59:59',
		@reportMM TINYINT=7
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			p.PID=peo.id								
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportMonth>@reportMM AND a.ReportMonth<13 AND a.ReportYear=2016 AND c.rf_idV006=4 AND p.PID IS NOT NULL
		AND c.Age>17
--------------------------------------------6---------------------------
INSERT #tPeople( rf_idCase ,DateBeg, PID,Sex,Age,rf_idV006,rf_idV008,AmountPayment, NumberCase,ReportMonth, ReportYear,C_POKL,DR,DS1)
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=d.rf_idCase
			AND d.TypeDiagnosis=1				           
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=2016 AND a.ReportMonth>@reportMM AND a.ReportMonth<13 AND c.rf_idV006 IN(1,2) 
