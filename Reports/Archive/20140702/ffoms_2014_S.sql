...
DECLARE @dateStart DATETIME='20140101',
		@dateEnd DATETIME='20141110',
		@dateEndRAK DATETIME='20141121',
		@reportMM TINYINT =10,
		@reportYYYY SMALLINT=2014,
		@codeM CHAR(6)='161007'
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			--AND c.DateEnd<'20140401'
			AND c.rf_idV006=1
					INNER JOIN dbo.t_People_Case pc ON
			c.id=pc.rf_idCase	
					INNER JOIN dbo.t_MES m ON
			c.id=m.rf_idCase
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportMonth>0 AND a.ReportMonth<=@reportMM AND a.ReportYear=@reportYYYY
-------------------------------------------update block----------------------------------
...
FROM #tPeople p INNER JOIN (SELECT rf_idCase,SUM(AmountDeduction) AS AmountDeduction 
							...
							WHERE DateRegistration>=@dateStart AND DateRegistration<@dateEndRAK
							GROUP BY rf_idCase
							) r ON
...
FROM(
	...
	WHERE t1.DateBegin>t.DateBegin AND t1.DateBegin<=DATEADD(dd,90,t.DateEnd) AND ISNULL(t.AmountRAK,'-1')>0 AND ISNULL(t1.AmountRAK,'-1')>0 
