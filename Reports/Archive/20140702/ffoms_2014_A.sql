...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=m.rf_idCase
WHERE f.DateRegistration>'20140101' AND f.DateRegistration<'20140502' AND a.ReportMonth>0 AND a.ReportMonth<4 AND a.ReportYear=2014 
		AND m.MES LIKE '2.78.%'
-------------------------------------------update block----------------------------------
...
FROM #tPeople p INNER JOIN (SELECT rf_idCase,SUM(AmountDeduction) AS AmountDeduction 
							FROM dbo.t_PaymentAcceptedCase a INNER JOIN (VALUES('A'),('G')) l(Letter) ON
														a.Letter=l.Letter 
							WHERE DateRegistration>='20140101' AND DateRegistration<'20140702'
							GROUP BY rf_idCase
							) r ON
