...
DECLARE @dtStart DATETIME='20170101',  --дата начало регистрации
		@dtEnd DATETIME='20170729 23:59:59',--дата окончания регистрации
		@dtEndRAK DATETIME='20170729 23:59:59',--дата окончания регистрации РАК
		@reportMM TINYINT=6,--указываете последний месяц из отчетного периода
		@reportYear SMALLINT=2017  --отчетный год
...
CREATE TABLE #tPeople(rf_idCase BIGINT,					  					  
					  ...
					  AmountRAK DECIMAL(11,2) NOT NULL DEFAULT(0.0),
					  DS varchar(15),
					  DS1 VARCHAR(15)					  	
					  )
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
				  INNER JOIN (VALUES('34001'),('34002'),('34006'),('34007')) v(CodeSMO) ON
...
WHERE f.DateRegistration>@dtStart AND f.DateRegistration<@dtEnd AND a.ReportMonth>0 AND a.ReportMonth<=@reportMM AND a.ReportYear=@reportYear  AND rf_idV006=1
...
ON [dbo].#tPeople ([AmountPayment])
INCLUDE ([rf_idCase],[DS])
-------------------------------------------update block----------------------------------
...
SELECT d.groupName,COUNT(DISTINCT rf_idCase) AS CountCase,CAST(SUM(p.AmountPayment) AS MONEY) AS AmountRAK
