USE AccountOMS
GO
DECLARE @codeM CHAR(6)='451001',--менять код МО
		@reportYear SMALLINT=2015,--менять отчетный год
		@dateStart DATETIME='20150101',--менять дату начало регистрации
		@dateEnd DATETIME='20150731 23:59:59',--менять дату окончания регистрации счетов
		@dateEndPay DATETIME='20150807',--менять дату окончания регистрации РАК
		@reportMM tinyint=12 --менять конечный месяц.
...
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportMonth>0 AND a.ReportMonth<=@reportMM AND a.ReportYear=@reportYear 
