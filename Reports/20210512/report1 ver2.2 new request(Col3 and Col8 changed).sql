...
WHERE d.ReportYear=@reportYear and NOT EXISTS(SELECT 1 FROM PolicyRegister.dbo.PEOPLE p WHERE p.ENP=d.enp AND ISNULL(p.DS,'22220101')<@firstDayNextMonth ) AND d.flag<28
UNION ALL
...
,cteB
AS
(SELECT ROW_NUMBER() OVER(PARTITION BY enp,flag ORDER BY age desc) AS idRow, c.ENP,c.flag,c.Age,c.Sex FROM cteCol4 c)
...
WHERE t.sid IS NOT NULL AND t.USL_OK<4 AND t.AmountPayment>0 AND t.ReportYear=@reportYear
		AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.ReportYear=@reportYear and d.enp=t.ENP AND d.flag=t.flag
						UNION ALL 
...
FROM #t t INNER JOIN dbo.t_PurposeOfVisit p on
		t.rf_idCase=p.rf_idCase
WHERE t.sid IS NOT NULL AND t.TypeFile='H' AND p.rf_idV025='1.3' AND t.USL_OK=3 AND p.DN =2 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.ReportYear=@reportYear and d.enp=t.ENP AND d.flag=t.flag) AND t.AmountPayment>0 AND t.ReportYear=@reportYear
UNION ALL
SELECT DISTINCT enp,flag,t.Age,t.Sex
FROM #t t INNER JOIN dbo.t_Case p on
		t.rf_idCase=p.id
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp=2 AND t.TypeDs=1 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.ReportYear=@reportYear AND d.enp=t.ENP AND d.flag=t.flag) AND t.AmountPayment>0 AND t.ReportYear=@reportYear
UNION ALL
SELECT DISTINCT enp,t.flag,t.Age,t.Sex
FROM  dbo.t_DS2_Info p 	INNER JOIN oms_nsi.dbo.tmp_DS2report202007 d ON
             p.DiagnosisCode=d.КОД	
           ...
           p.rf_idCase=t.rf_idCase
		   AND d.flag=t.flag
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp=2 AND t.TypeDs=2 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.ReportYear=@reportYear and d.enp=t.ENP AND d.flag=t.flag) AND t.AmountPayment>0 AND t.ReportYear=@reportYear AND d.flag<28
)
...
SELECT flag,COUNT(DISTINCT enp) AS Col8
		...
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 60 THEN c.ENP ELSE NULL end) AS Col11
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=60 THEN c.ENP ELSE NULL end) AS Col12
FROM (SELECT distinct flag,ENP,Age,sex FROM dbo.T_inform202007 WHERE ReportYear=@reportYear AND NotificationDate<@DayNextMonth and sex IS NOT NULL AND flag<28) c
GROUP BY flag
-------------------------------------28 and 38-----------------------------------
;WITH cte
AS
...
FROM dbo.DNPersons202007 d WHERE d.ReportYear=@reportYear AND flag<28
UNION ALL
...
AS
(SELECT ROW_NUMBER() OVER(PARTITION BY enp,flag ORDER BY age desc) AS idRow, c.ENP,c.flag,c.Age,c.Sex FROM cte c)
