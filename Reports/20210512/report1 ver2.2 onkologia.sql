...
DECLARE @dateStartReg DATETIME='20210101',
		...
		@dateEndRegRAK DATETIME,--будут говорить конечную дату, со слов С.Б. Никитенко
		@reportYear SMALLINT=2021,
		@reportMonth TINYINT=4,
		@dateForAge DATE='20211201'

SELECT @dateEndReg=CAST(@reportYear AS CHAR(4))+RIGHT('0'+CAST(@reportMonth+1 AS VARCHAR(2)),2)+'16',
	@dateEndRegRAK=CAST(@reportYear AS CHAR(4))+RIGHT('0'+CAST(@reportMonth+1 AS VARCHAR(2)),2)+'17'


declare @firstDayNextMonth DATE,
		@DayNextMonth DATE
SET @firstDayNextMonth=(CASE WHEN @reportMonth>1 THEN DATEADD(MONTH,1,'2021'+RIGHT('0'+CAST(@reportMonth-1 AS VARCHAR(2)),2)+'01') ELSE '20210101' END) 
SET @DayNextMonth=DATEADD(MONTH,1, @firstDayNextMonth)
--только онкология. формируем по ней используя старый алгоритм. т.к. Гайдарова обещала переписать его
SELECT DISTINCT c.id AS rf_idCase, cc.AmountPayment,f.CodeM,p.ENP,DATEDIFF(YEAR,pp.BirthDay,@dateForAge) AS Age,c.rf_idV006 AS USL_OK,c.rf_idRecordCasePatient,pp.rf_idV005 AS Sex, d.flag,f.TypeFile,@firstDayNextMonth AS dd, 1 AS TypeDs
INTO #t
...
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@reportYear AND a.ReportMonth<=@reportMonth  AND  a.rf_idSMO<>'34'  AND dd.TypeDiagnosis=1 AND d.flag>27

--CREATE UNIQUE NONCLUSTERED INDEX IXCase ON #t(rf_idCase,flag,ENP) WITH IGNORE_DUP_KEY

INSERT #t
SELECT DISTINCT c.id AS rf_idCase, cc.AmountPayment,f.CodeM,p.ENP,DATEDIFF(YEAR,pp.BirthDay,@dateForAge) AS Age,c.rf_idV006 AS USL_OK,c.rf_idRecordCasePatient,pp.rf_idV005 AS Sex, d.flag,f.TypeFile,@firstDayNextMonth AS dd,2
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
					INNER JOIN dbo.t_RecordCasePatient r ON
			a.id=r.rf_idRegistersAccounts
			...
			c.id=dd.rf_idCase																
					INNER JOIN oms_nsi.dbo.tmp_DS2report202007 d ON
			dd.DiagnosisCode=d.КОД
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@reportYear AND a.ReportMonth<=@reportMonth AND  a.rf_idSMO<>'34' AND f.TypeFile='F' AND dd.TypeDiagnosis<>1 AND d.flag>27
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=dd.rf_idCase					
					INNER JOIN oms_nsi.dbo.tmp_DS2report202007 d ON
             dd.DiagnosisCode=d.КОД											
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@reportYear AND a.ReportMonth<=@reportMonth AND  a.rf_idSMO<>'34' AND f.TypeFile='F' AND d.flag>27
PRINT('------------------------------------------------------------------------------')
...
WHERE d.ReportYear=@reportYear and NOT EXISTS(SELECT 1 FROM PolicyRegister.dbo.PEOPLE p WHERE p.ENP=d.enp AND ISNULL(p.DS,'22220101')<@firstDayNextMonth) AND d.flag>27
UNION ALL
SELECT DISTINCT enp,flag,t.Age,sex
FROM #t t INNER JOIN dbo.t_PurposeOfVisit p on
		t.rf_idCase=p.rf_idCase
WHERE t.sid IS NOT NULL AND t.TypeFile='H' AND t.USL_OK<=3 AND t.AmountPayment>0 AND flag>27
UNION ALL
SELECT DISTINCT enp,flag,t.Age,t.Sex
FROM #t t INNER JOIN dbo.t_Case p on
		t.rf_idCase=p.id
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND t.TypeDs=1 AND t.AmountPayment>0 AND flag>27
UNION ALL
...
FROM  dbo.t_DS2_Info p 	INNER JOIN oms_nsi.dbo.tmp_DS2report202007 d ON
		   ...
		   AND d.flag=t.flag
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 /*AND p.IsNeedDisp IN(1,2)*/ AND t.TypeDs=2 AND t.AmountPayment>0 AND d.flag>27
)
...
SELECT Col1,col3,col4,col5,col6,col7 FROM #tCase
-----------------------------------------------------9--------------------------------------------------------------------------
;WITH cteCol9
AS(
SELECT DISTINCT enp,flag,t.Age,sex
FROM #t t INNER JOIN dbo.t_PurposeOfVisit p on
		t.rf_idCase=p.rf_idCase
WHERE t.sid IS NOT NULL AND t.TypeFile='H' AND p.rf_idV025='1.3' AND t.USL_OK=3 AND p.DN =2 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP AND d.flag=t.flag AND d.ReportYear=@reportYear) AND t.AmountPayment>0
UNION ALL
SELECT DISTINCT enp,flag,t.Age,t.Sex
FROM #t t INNER JOIN dbo.t_Case p on
		t.rf_idCase=p.id
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp=2 AND t.TypeDs=1 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP AND d.flag=t.flag AND d.ReportYear=@reportYear) AND t.AmountPayment>0
UNION ALL
SELECT DISTINCT enp,t.flag,t.Age,t.Sex
FROM  dbo.t_DS2_Info p 	INNER JOIN oms_nsi.dbo.tmp_DS2report202007 d ON
             p.DiagnosisCode=d.КОД	
           ...
           p.rf_idCase=t.rf_idCase
		   AND d.flag=t.flag
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp=2 AND t.TypeDs=2 AND NOT EXISTS(SELECT 1 FROM dbo.DNPersons202007 d WHERE d.enp=t.ENP AND d.flag=t.flag AND d.ReportYear=@reportYear) AND t.AmountPayment>0 AND d.flag>27
)
...
SELECT flag,COUNT(DISTINCT enp) AS Col8
		...
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 60 THEN c.ENP ELSE NULL end) AS Col11
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=60 THEN c.ENP ELSE NULL end) AS Col12
FROM (SELECT distinct flag,ENP,Age,sex FROM dbo.T_inform202007 WHERE  ReportYear=@reportYear AND NotificationDate<@DayNextMonth AND sex IS NOT NULL AND flag>27) c
GROUP BY flag
-------------------------------------28 and 38-----------------------------------
;WITH cte
AS
...
WHERE p.rf_idV025='1.3' AND m.MUGroupCode=2 AND  m.MUUnGroupCode=88 AND rf_idDepartmentMO IS NULL AND t.TypeFile='H' AND t.AmountPayment>0
)
...
FROM #t t INNER JOIN dbo.t_PurposeOfVisit p on
		t.rf_idCase=p.rf_idCase
			INNER JOIN dbo.t_Meduslugi m ON
		t.rf_idCase=m.rf_idCase
WHERE  p.rf_idV025='1.3' AND m.MUGroupCode=2 AND  m.MUUnGroupCode=88 AND rf_idDepartmentMO=0 AND t.TypeFile='H' AND t.AmountPayment>0
)
...
FROM dbo.DNPersons202007 d WHERE d.ReportYear=@reportYear and d.flag>27
UNION ALL
...
FROM #t t INNER JOIN dbo.t_Case p on
		t.rf_idCase=p.id
WHERE t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp IN(1,2) AND t.TypeDs=1 AND t.AmountPayment>0
UNION ALL
...
FROM  dbo.t_DS2_Info p 	INNER JOIN oms_nsi.dbo.tmp_DS2report202007 d ON
           ...
           p.rf_idCase=t.rf_idCase
		   AND d.flag=t.flag
WHERE t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp IN(1,2) AND t.TypeDs=2 AND t.AmountPayment>0 AND d.flag>27
)
...
(
	...
	FROM #t t INNER JOIN #tStacSkoray s ON
			t.enp=s.ENP
			AND t.flag=s.flag
	WHERE t.USL_OK<3 --AND t.AmountPayment>0
)
...
FROM #tStacSkoray tt INNER JOIN #t t ON
		...
		t.rf_idCase=c.id
WHERE c.rf_idV009 IN(105,106,205,206,313,405,406,411) AND t.AmountPayment>0 AND t.TypeDs=1
)
...
(
	...
	FROM #t t INNER JOIN #tStacSkoray s ON
			t.enp=s.ENP
			AND t.flag=s.flag
	WHERE t.USL_OK=4 --AND t.AmountPayment>0
)
...
FROM #tCase WHERE Col1>27 ----убираем всю онкологию
GROUP BY col1
ORDER BY col1
GO
DROP TABLE #t
