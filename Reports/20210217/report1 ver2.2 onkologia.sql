...
DECLARE @dateStartReg DATETIME='20210101',
		@dateEndReg DATETIME='20210216',
		@dateStartRegRAK DATETIME='20210101',
		@dateEndRegRAK DATETIME='20210217',--будут говорить конечную дату, со слов С.Б. Никитенко
...
WHERE t.sid IS NOT NULL AND t.TypeFile='H' AND p.rf_idV025='1.3' AND t.USL_OK=3 AND p.DN IN(1,2) AND t.AmountPayment>0 AND flag>27
UNION ALL
SELECT DISTINCT enp,flag,t.Age,t.Sex
FROM #t t INNER JOIN dbo.t_Case p on
		t.rf_idCase=p.id
WHERE t.sid IS NOT NULL AND t.TypeFile='F' AND t.USL_OK=3 AND p.IsNeedDisp IN(1,2) AND t.TypeDs=1 AND t.AmountPayment>0 AND flag>27
UNION ALL
...
SELECT flag,COUNT(DISTINCT enp) AS Col8
		...
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 60 THEN c.ENP ELSE NULL end) AS Col11
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=60 THEN c.ENP ELSE NULL end) AS Col12
FROM (SELECT distinct flag,ENP,Age,sex FROM dbo.T_inform202007 WHERE  ReportYear=@reportYear AND sex IS NOT NULL AND flag>27) c
GROUP BY flag
-------------------------------------28 and 38-----------------------------------
;WITH cte
AS
