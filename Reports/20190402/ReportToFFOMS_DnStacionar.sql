...
SELECT DISTINCT c.id AS rf_idCase, c.AmountPayment,ps.ENP,m.MES, c.AmountPayment AS AmountPay ,a.ReportYear
		, CASE WHEN m.MES IN('2016.0','2054.0','2055.0','2056.0','2057.0','2058.0','2059.0','2060.0','2061.0') THEN 2 				
				WHEN m.mes IN('2046.0','2047.0','2048.0') THEN 3 
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=mm.rf_idCase													   					  					      
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=@reportYear AND c.rf_idV006=2 AND f.TypeFile='H' AND c.rf_idV010<>32 
----------------------------------ВМП 2018---------------------------------------
INSERT #tCases
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=u.rf_idCase					   					  					      
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=2019 AND c.rf_idV006=2 AND a.ReportMonth<3 AND f.TypeFile='H' AND c.rf_idV010<>32
----------------------------------------2019 ВМП-----
INSERT #tCases
SELECT c.id AS rf_idCase, c.AmountPayment,ps.ENP,m.MES, c.AmountPayment AS AmountPay ,a.ReportYear
		, CASE WHEN mm.MUGroupCode=1 AND mm.MUUnGroupCode=12 AND mm.MUCode=356 THEN 2 ELSE 1 END AS USL_TIP
...
/*
SELECT rf_idCase,'ONK' AS TypeTbale INTO #t260 FROM dbo.t_260order_ONK WHERE USL_OK=2
UNION all
SELECT rf_idCase ,'VMP' FROM dbo.t_260order_VMP WHERE USL_OK=2
...
WHERE m.MES='ds19.028' AND AmountPay>0
