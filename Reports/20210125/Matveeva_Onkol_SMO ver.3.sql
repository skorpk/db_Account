...
CREATE TABLE #tmpDrug(rf_idCase bigint,rf_idV024 VARCHAR(30),MNN VARCHAR(2000))

INSERT #tmpDrug
SELECT distinct d.rf_idCase,d.rf_idV024,MNN=REPLACE((SELECT DISTINCT ISNULL(n20.MNN,'') AS 'data()' 
...
FROM #t t INNER JOIN t_DrugTherapy d ON
...
FROM #t t left JOIN dbo.t_AdditionalCriterion a ON
		t.rf_idCase=a.rf_idCase
			left JOIN (SELECT * FROM #tmpDrug	) d ON
          t.rf_idCase=d.rf_idCase
