USE AccountOMS
go
DECLARE @dateEndReg DATETIME='20210416',
		@reportYear SMALLINT=2021,
		@reportMonth TINYINT=3

DECLARE @dateStartReg DATETIME=CAST(@reportYear AS CHAR(4))+'0101'
...
WHERE cv.UnitCode IN(30,31,32,38,145,146,147,261,262,318,319,320,321,322,323)
GROUP BY c.CodeM

SELECT cv.UnitCode ,u.unitName,SUM(cv.Qunatity)
FROM #tCases c JOIN dbo.t_Case_UnitCode_V006 cv ON
		c.rf_idCase=cv.rf_idCase
				JOIN oms_nsi.dbo.tPlanUnit u ON
       cv.UnitCode=u.unitCode
WHERE c.USL_OK=3 AND cv.UnitCode IN(30,31,32,38,145,146,147,261,262,318,319,320,321,322,323)
GROUP BY cv.UnitCode ,u.unitName
ORDER BY cv.unitCode
/*
----------------------------------------------------------Col1------------------------------------------
