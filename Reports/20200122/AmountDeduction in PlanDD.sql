...
SELECT c.id AS rf_idCase, r.ENP,c.AmountPayment,f.CodeM,a.Account,f.DateRegistration,c.DateEnd,c.idRecordCase,a.DateRegister,a.rf_idSMO
INTO #tCases
...
ALTER TABLE #tCases ADD AmountDeduction2 DECIMAL(15,2) NOT NULL DEFAULT(0.0)		


UPDATE p SET p.AmountDeduction=r.AmountDeduction
FROM #tCases p INNER JOIN (SELECT c.rf_idCase,SUM(c.AmountDeduction) AS AmountDeduction
							FROM dbo.t_PaymentAccepted c
							WHERE c.DateRegistration>=@dateStart AND c.DateRegistration<@dateEnd
							GROUP BY c.rf_idCase
							) r ON
			p.rf_idCase=r.rf_idCase

UPDATE p SET p.AmountDeduction2=r.AmountDeduction
FROM #tCases p INNER JOIN (SELECT c.rf_idCase,SUM(c.AmountDeduction) AS AmountDeduction
							FROM dbo.tmp_PaymentAcceptedCase2 c
							WHERE c.DateRegistration>=@dateStart AND c.DateRegistration<@dateEnd
							GROUP BY c.rf_idCase
							) r ON
			p.rf_idCase=r.rf_idCase
...
SELECT rf_idSMO,SUM(AmountDeduction), SUM(AmountDeduction2)
FROM #tCases 
GROUP BY rf_idSMO
