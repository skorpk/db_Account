...
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@reportYear  AND f.TypeFile='F'
	 AND c.rf_idV006 =3  AND c.Age>17 AND a.rf_idSMO<>'34'
...
FROM (
		...
		FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
                    ...
                    d.DiagnosisCode=k.DiagnosisCode
		             ...
		             p.enp=t.ENP				 
		WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@reportYear AND f.TypeFile='H'
				 AND c.rf_idV006 =3 AND pv.rf_idV025='1.3' AND c.Age>17 AND a.rf_idSMO<>'34' AND c.rf_idV002 =60 --AND [sid] IS NOT NULL
		UNION ALL
		...
		FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
		            ...
		            r.id=cc.rf_idRecordCasePatient										
		             ...
		             p.enp=t.ENP				 
		WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@reportYear AND f.TypeFile='H'
				 AND c.rf_idV006 <3 AND c.Age>17 AND a.rf_idSMO<>'34' AND c.rf_idV002 =60 --AND [sid] IS NOT NULL
		) tt
)
SELECT q.IdRow,
       q.ENP,
