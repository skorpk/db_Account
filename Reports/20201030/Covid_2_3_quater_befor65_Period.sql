...
INSERT #csg(code,typeHardCase,PeriodUse,crit) VALUES('st12.008.1',1,2,null),('st12.009.1',1,2,null),('st23.004.1',1,2,'cr8'),('st12.013.2',2,2,'cr4'),('st12.013.1',3,2,'cr5'),('st12.013.1',4,2,'cr6')
...
SELECT count(DISTINCT rf_idCompletedCasse) FROM #tCases
...
SELECT COUNT(DISTINCT rf_idCompletedCasse)
FROM #tCases WHERE AmountPayment>0.0
...
SELECT IsTypeHardCase,COUNT(rf_idCompletedCasse) FROM #tCases GROUP BY IsTypeHardCase

SELECT rf_idCompletedCasse FROM #tCases GROUP BY rf_idCompletedCasse HAVING COUNT(*)>1

SELECT 'Без степени до 65',@quater,COUNT(DISTINCT rf_idCompletedCasse) FROM #tCases WHERE IsTypeHardCase=0
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
            ...
            ps.ENP=s.ENP
					INNER JOIN dbo.t_Case c ON
			r.id=c.rf_idRecordCasePatient							
WHERE f.DateRegistration>'20200301' AND f.DateRegistration<@dateEnd AND a.ReportYear=2020 AND a.ReportMonth>1 AND a.ReportMonth<10 AND c.rf_idV006=3 AND a.rf_idSMO<>'34' 
		AND c.rf_idV002 IN(28, 57, 58, 65, 75, 97, 151,160)
PRINT('AMP')
