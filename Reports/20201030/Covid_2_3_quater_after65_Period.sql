...
WHERE f.DateRegistration>=@dateStart AND f.DateRegistration<@dateEnd AND a.ReportYear=2020 AND  a.ReportMonth>@startMonth AND a.ReportMonth<@endMonth AND c.rf_idV006=1
	AND d.TypeDiagnosis IN(1,3) AND d.DiagnosisCode IN('U07.1','U07.2') AND a.rf_idSMO<>'34' AND c.Age>65
...
SELECT 'Без степени после 65',@quater,COUNT(DISTINCT rf_idCompletedCasse) FROM #tCases WHERE IsTypeHardCase=0
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
            ...
            ps.ENP=s.ENP
					INNER JOIN dbo.t_Case c ON
			r.id=c.rf_idRecordCasePatient							
WHERE f.DateRegistration>'20200301' AND f.DateRegistration<@dateEnd AND a.ReportYear=2020 AND a.ReportMonth>1 AND a.ReportMonth<10 AND c.rf_idV006=3
	AND a.rf_idSMO<>'34' AND c.rf_idV002 IN(28, 57, 58, 65, 75, 97, 151,160)
...
------------------------------------Вывод итоговых данных возрастная группа старше 65---------------------
