USE [PlanDD]
GO
IF OBJECT_ID('usp_173N_Table2',N'P') IS NOT NULL
	DROP PROCEDURE usp_173N_Table2
GO
CREATE PROCEDURE [dbo].[usp_173N_Table2]
...
CREATE UNIQUE NONCLUSTERED INDEX IX_1 ON #t(ENP) INCLUDE(PID,[sid]) WITH IGNORE_DUP_KEY
...
SELECT l.CodeM,l.NameS,ISNULL(t.Col1,0) AS Col1,ISNULL(t.Col2,0) AS Col2
FROM #LPU lp JOIN vw_sprT001 l ON
		l.CodeM = lp.CodeM
			LEFT join (
						SELECT p.CODEM,COUNT(DISTINCT CASE when p.ReportMonth=@reportMonth THEN p.ENP ELSE NULL end) AS col1, COUNT(DISTINCT p.ENP) AS col2
						FROM dbo.t_Case_173N_Profil p JOIN #t t ON
						             p.enp=t.ENP				 				
						WHERE p.DateRegistration>=@dateStartReg AND p.DateRegistration<@dateEndReg  AND p.ReportYear=@reportYear AND p.ReportMonth<=@reportMonth
							  AND [sid] IS NOT NULL 
						GROUP BY p.codem
						) t ON
        lp.CodeM=t.CodeM
