...
SET @firstDayNextMonth=DATEADD(MONTH,1,'2021'+RIGHT('0'+CAST(@reportMonth-1 AS VARCHAR(2)),2)+'01')
...
SELECT flag,COUNT(DISTINCT enp) AS Col8
		...
		,COUNT(distinct CASE WHEN c.Sex=2 AND age< 60 THEN c.ENP ELSE NULL end) AS Col11
		,COUNT(distinct CASE WHEN c.Sex=2 AND age>=60 THEN c.ENP ELSE NULL end) AS Col12
FROM (SELECT distinct flag,ENP,Age,sex FROM dbo.T_inform202007 WHERE sex IS NOT NULL AND flag>27) c
GROUP BY flag
-------------------------------------28 and 38-----------------------------------
;WITH cte
AS
