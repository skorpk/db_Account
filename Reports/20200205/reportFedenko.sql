...
SELECT * FROM oms_nsi.dbo.V001 WHERE IDRB='A16.10.021.001'
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=m.rf_idCase					
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@reportYear AND c.rf_idV006=1 AND m.MUSurgery='A16.10.021.001'		
UNION ALL
SELECT DISTINCT c.id AS rf_idCase, c.AmountPayment,p.ENP, a.ReportYear,a.rf_idSMO
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
			...
			r.id=p.rf_idRecordCasePatient					
					INNER JOIN dbo.t_Meduslugi m ON
			c.id=m.rf_idCase					
WHERE f.DateRegistration>=@dateStartReg2 AND f.DateRegistration<@dateEndReg2  AND a.ReportYear=@reportYear2 AND c.rf_idV006=1 AND m.MUSurgery='A16.10.021.001'
