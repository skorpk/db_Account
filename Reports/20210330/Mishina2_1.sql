...
SELECT DISTINCT c1.rf_idCase AS rf_idCaseCross,c1.FIO AS FIOCross,c1.BirthDay AS DRCross,c1.ENP , c1.NumberPolis AS NumberPolicyCross,
       c1.DateRegistration AS DateRegistrationCross,c1.Account AS AcountCross,c1.DateAccount AS DateAccountCross,c1.idRecordCase AS idRecordCaseCross,
...
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg   AND a.ReportYearMonth BETWEEN 202004 AND 202103 
	AND c1.DateBegin<cc.DateEnd AND c1.DateEnd>cc.DateBegin AND c1.rf_idCase<>c.id AND c.rf_idV006<4    
-------------------------------------------------------------------------------------------------------------

SELECT DISTINCT c1.FIOCross,c1.DRCross,c1.ENP , c1.NumberPolicyCross,
		CAST(c1.DateRegistrationCross AS DATE),c1.AcountCross,c1.DateAccountCross,c1.idRecordCaseCross,
       c1.AmountPaymentCross,c1.DiagCross,c1.ProfilCross,c1.FOR_POMCross,c1.DateBeginCross,
       c1.DateEndCross,c1.PRVSCross,c1.CSGCross,c1.RSLTCross,c1.P_PERCross
	   --------------------------------------
	   ...
	   ,v6.name AS USL_OK
	   ,RTRIM(d.DS1)+' - '+mkb.Diagnosis AS Diag,v2.name AS Profil,v14.name AS FOR_POM ,cc.DateBegin,cc.DateEnd
	   ,v4.name AS PRVS,m.MES+' - '+sM.CSG AS CSG
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
            ...
            c.rf_idV006=v6.id					
					JOIN dbo.vw_sprV009 v9 ON
             c.rf_idV009=v9.id						    					
					JOIN dbo.t_Meduslugi mm ON
            c.id=mm.rf_idCase
			AND mm.Price>0
					JOIN dbo.vw_sprMU  mu ON
            mm.MU=mu.MU
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg   AND a.ReportYearMonth BETWEEN 202004 AND 202103 AND c.IsCompletedCase=0
ORDER BY c1.FIOCross
GO
DROP TABLE #tCases1
GO
DROP TABLE #tCases
