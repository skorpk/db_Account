...
CREATE TABLE #tTotal(idrow TINYINT, ColName VARCHAR(15),rf_idMO CHAR(6),Col1 INT,Col2 INT, Col4 DECIMAL(15,5),Col5 INT,Col6 INT, Col7 DECIMAL(15,5),Col8 DECIMAL(15,5))
...
--------------------------------------------------------------MU 60.3.*-----------------------------------
INSERT #tTotal
SELECT DISTINCT 12 AS IdRow,'2.2.9' AS ColName,c.rf_idMO AS MCOD
	,0 AS Col1,CAST(NULL AS INT) AS Col2,0.0 AS Col4
...
FROM #tCases c INNER JOIN #tLPU l ON
		l.mcod = c.rf_idMO
				INNER JOIN (SELECT rf_idCase,SUM(Quantity) AS Quantity FROM dbo.t_Meduslugi m WHERE  m.MUGroupCode=2 AND m.MUUnGroupCode IN(76,79,81,88) GROUP BY rf_idCase,MU) m on 
		m.rf_idCase=c.rf_idCase 		
...
WHERE  c.rf_idSMO<>'34'
INSERT #tTotal
SELECT DISTINCT 13 AS IdRow,'2.2.10' AS ColName,c.rf_idMO AS MCOD
	,0 AS Col1,CAST(NULL AS INT) AS Col2,0.0 AS Col4
...
FROM #tCases c INNER JOIN dbo.t_MES m on 
		m.rf_idCase=c.rf_idCase 				
WHERE c.TypeCase=2  AND c.rf_idSMO<>'34' AND m.mes IN('2.78.90','2.78.91')
INSERT #tTotal
SELECT  14 AS IdRow,'2.2.11' AS ColName,c.rf_idMO AS MCOD
	,0 AS Col1,CAST(NULL AS INT) AS Col2,0.0 AS Col4
	,1 AS Col5
...
SELECT  14 AS IdRow,'2.2.11' AS ColName,c.rf_idMO AS MCOD
	...
	,CASE WHEN rf_idSMO='34' THEN AmountPayment ELSE 0.0 END AS Col8
FROM #tCases c 
WHERE c.TypeCase=2  AND EXISTS(SELECT 1 FROM dbo.t_Meduslugi m INNER JOIN #tMU mm ON mm.MU = m.MU WHERE m.rf_idCase=c.rf_idCase AND mm.TypeMU=4)
INSERT #tTotal
-----------------------------------------------------------------2019---------------------------------------------------
...
------------------------------------MU 60.3.*-----------------------------------
INSERT #tTotal
SELECT DISTINCT 112 AS IdRow,'2.2.9' AS ColName,c.rf_idMO AS MCOD
	,0 AS Col1,CAST(NULL AS INT) AS Col2,0.0 AS Col4
	,CASE WHEN rf_idSMO<>'34' THEN 1 ELSE NULL END AS Col5
	,CASE WHEN rf_idSMO='34' THEN 1 ELSE NULL END AS Col6
	,CASE WHEN rf_idSMO<>'34' THEN AmountPayment ELSE 0.0 END AS Col7
...
FROM #tTotal c INNER JOIN (SELECT DISTINCT mcod,LPU_Mcode FROM dbo.vw_sprT001) l ON
		c.rf_idMO=l.MCOD			
