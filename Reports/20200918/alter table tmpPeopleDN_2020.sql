...
/*
alter TABLE tmpPeopleDN_2020  ADD id int not null IDENTITY(1,1)
ALTER TABLE tmpPeopleDN_2020 ADD MainDS VARCHAR(4)
ALTER TABLE tmpPeopleDN_2020 ADD idRow int
ALTER TABLE tmpPeopleDN_2020 ADD IsDNType tinyint not null default (1)

ALTER TABLE tmpPeopleDN_2020 ADD LPU CHAR(6)
ALTER TABLE tmpPeopleDN_2020 ADD Q CHAR(5)
ALTER TABLE tmpPeopleDN_2020 ADD PID INT
ALTER TABLE tmpPeopleDN_2020 ADD [sid] INT
ALTER TABLE tmpPeopleDN_2020 ADD [lid] INT

ALTER TABLE tmpPeopleDN_2020 ADD dd date not null default('20200701')

ALTER TABLE tmpPeopleDN_2020 ADD Col8 tinyint

ALTER TABLE tmpPeopleDN_2020 ADD Col9 date
*/
-----------------обновляем рубрику
/*
UPDATE p SET p.MainDS=m.MainDS
FROM dbo.tmpPeopleDN_2020 p INNER JOIN dbo.vw_sprMKB10 m ON
...
FROM cte c INNER JOIN dbo.tmpPeopleDN_2020 p ON
		p.id = c.id

UPDATE p SET p.w=r.rf_idV005
FROM dbo.tmpPeopleDN_2020 p INNER JOIN dbo.t_Case c ON
			p.rf_idCase=c.id
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			...
			c.id=dd.rf_idCase				
					INNER JOIN dbo.tmpPeopleDN_2020 pp ON
			p.enp=pp.ENP		
...
update tmpPeopleDN_2020 set IsDNType=1 where IsDNType is null

*/
UPDATE p SET p.PID=pp.id
FROM dbo.tmpPeopleDN_2020 p INNER JOIN PolicyRegister.dbo.PEOPLE pp ON
			p.ENP=pp.ENP
WHERE p.idRow=1 AND p.PID IS NULL

UPDATE p SET p.PID=pp.PID
FROM dbo.tmpPeopleDN_2020 p INNER JOIN PolicyRegister.dbo.HISTENP pp ON
			p.ENP=pp.ENP
WHERE p.idRow=1
...
if Object_ID(N'tempdb..#t_giplpu_1', N'U') is not null drop table [#t_giplpu_1];
	...
	select p.id pid, t.dd dd, s.id [sid], s.q
		into #t_giplpu_1 
		from PolicyRegister.dbo.polis s join PolicyRegister.dbo.people p ON s.pid=p.id inner join tmpPeopleDN_2020 t ON s.pid=t.pid                                                                  
	...
	update tmpPeopleDN_2020 set [sid]=#t_giplpu_1.[sid], q=#t_giplpu_1.q  from tmpPeopleDN_2020 join #t_giplpu_1 on(tmpPeopleDN_2020.pid=#t_giplpu_1.pid);
	update tmpPeopleDN_2020 set lid=#t_giplpu_2.lid, lpu=#t_giplpu_2.lpu  from tmpPeopleDN_2020 join #t_giplpu_2 on(tmpPeopleDN_2020.pid=#t_giplpu_2.pid);
...
FROM tmpPeopleDN_2020 t INNER JOIN PolicyRegister.dbo.PEOPLE p ON
		t.pid=p.id
WHERE p.DS IS NOT NULL AND p.ds>='20200101' AND p.DS<'20200701'

--UPDATE tmpPeopleDN_2020 SET Col9=NULL WHERE Col8=2 AND idRow=1

BEGIN TRANSACTION
UPDATE t SET Col8=2,col9=p.DSTOP
FROM tmpPeopleDN_2020 t INNER JOIN (SELECT PID, MAX(DSTop) DSTop FROM PolicyRegister.dbo.POLIS GROUP BY PID) p ON
		t.pid=p.pid
...
update tmpPeopleDN_2020 SET idRow=null WHERE Col9<'20200101' 
go