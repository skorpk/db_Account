...
DECLARE @dateStartReg DATETIME='20200701',
		@dateEndReg DATETIME='20200816',
		@dateStartRegRAK DATETIME='20200701',
		@dateEndRegRAK DATETIME='20200816',
...
INSERT #tParams(dateStartReg,dateEndReg,dateEndRegRAK,reportYear,reportMonth,reportPeriod)
VALUES(  @dateStartReg , @dateEndReg ,@dateEndRegRAK,@reportYear,@reportMonth, @reportPeriod)
...
SELECT @dateStartReg=dateStartReg,@dateEndReg=dateEndReg,@dateEndRegRAK=dateEndRegRAK,@reportYear=reportYear,@reportMonth=reportMonth,@reportPeriod =reportPeriod FROM #tParams
...
SELECT 11 AS IdRow,'2.2.8' AS ColName,c.rf_idMO AS MCOD
	...
	,CASE WHEN rf_idSMO<>'34' THEN AmountPayment ELSE 0.0 END AS Col7
	,CASE WHEN rf_idSMO='34' THEN AmountPayment ELSE 0.0 END AS Col8
FROM #tCases c INNER join (SELECT m.rf_idCase ,SUM(m.Quantity) Quantity FROM dbo.t_Meduslugi m WHERE m.MUGroupCode=4 AND m.MUUnGroupCode=17 AND m.MUCode IN(785,786) GROUP BY m.rf_idCase) m ON
			m.rf_idCase=c.rf_idCase 			
--------------------------------------------------------------MU 60.3.*-----------------------------------
UNION ALL
...
,COUNT(CASE WHEN dd.DiagnosisCode LIKE 'U%' THEN 1 ELSE NULL END) IsCOVID, a.rf_idSMO, a.rf_idMO	
INTO #tCases
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
					INNER JOIN dbo.t_RecordCasePatient r ON
            ...
            r.id=cc.rf_idRecordCasePatient
					INNER JOIN dbo.t_Diagnosis dd ON
			c.id=dd.rf_idCase						
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@reportYear AND a.ReportMonth>2 AND a.ReportMonth<=@reportMonth AND dd.TypeDiagnosis IN(1,3)
