...
CREATE TABLE #tCase
(IdRow VARCHAR(3),Col3 INT NOT NULL DEFAULT 0,Col4 numeric NOT NULL DEFAULT 0,Col5 INT NOT NULL DEFAULT 0,Col6 numeric NOT NULL DEFAULT 0,Col7 INT NOT NULL DEFAULT 0,
Col8 numeric NOT NULL DEFAULT 0,Col9 INT NOT NULL DEFAULT 0,Col10 numeric NOT NULL DEFAULT 0,Col11 INT NOT NULL DEFAULT 0,Col12 numeric NOT NULL DEFAULT 0,Col13 INT NOT NULL DEFAULT 0,Col14 numeric NOT NULL DEFAULT 0)
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
     ...
     --        c.id=m.rf_idCase
					INNER JOIN dbo.t_Case_UnitCode_V006  u ON
             c.id=u.rf_idCase
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@reportYear AND u.UnitCode IN(31,146) AND c.DateEnd BETWEEN @dateBegCase AND @dateEndCase
GROUP BY c.id, c.AmountPayment,a.ReportYear,c.Age,u.Qunatity
-----------------------------2019
SELECT @dateEndCase=DATEADD(YEAR,1,@dateEndCase),@dateBegCase=DATEADD(YEAR,1,@dateBegCase)
INSERT INTO #t
...
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
     ...
     --        c.id=m.rf_idCase
					INNER JOIN dbo.t_Case_UnitCode_V006  u ON
             c.id=u.rf_idCase								
WHERE f.DateRegistration>=@dateStartReg2 AND f.DateRegistration<@dateEndReg2  AND a.ReportYear=@reportYear2 AND u.UnitCode IN (31,146) AND c.DateEnd BETWEEN @dateBegCase AND @dateEndCase
GROUP BY c.id, c.AmountPayment,a.ReportYear,c.Age,u.Qunatity
...
INSERT INTO #t
SELECT DISTINCT c.id AS rf_idCase, c.AmountPayment AS AmountPayment2,c.AmountPayment,a.ReportYear,c.Age,c.rf_idRecordCasePatient AS Quantity
,CASE WHEN c.rf_idV002 IN(18,60,76) THEN 1 ELSE 0 END AS Profil
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
					INNER JOIN dbo.t_RecordCasePatient r ON
...
SELECT '3'
		...
		,SUM(CASE WHEN ReportYear=2018 AND Age>64 THEN AmountPayment ELSE 0 END) AS Col8
		----child 2019
		,count(DISTINCT CASE WHEN ReportYear=2019 AND Age<18 THEN Quantity ELSE 0 END) AS Col9
		,SUM(CASE WHEN ReportYear=2019 AND Age<18 THEN AmountPayment ELSE 0 END) AS Col10
		----Adult 2019
		,count(DISTINCT CASE WHEN ReportYear=2019 AND Age>17 AND Age<65 THEN Quantity ELSE 0 END) AS Col11
		,SUM(CASE WHEN ReportYear=2019  AND Age>17 AND Age<65 THEN AmountPayment ELSE 0 END) AS Col12
		---------------Retiree 2019
		,count(DISTINCT CASE WHEN ReportYear=2019 AND Age>64 THEN Quantity ELSE 0 END) AS Col13
		,SUM(CASE WHEN ReportYear=2019 AND Age>64 THEN AmountPayment ELSE 0 END) AS Col14
FROM #t
