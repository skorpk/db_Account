...
SELECT c.rf_idCase,r.SeriaPolis,r.NumberPolis,c.ENP,p.Fam,p.im,p.ot, p.BirthDay,p.BirthPlace,d.NumberDocument,d.SNILS,cc.DateEnd,cc.DateBegin,p.rf_idV005
FROM dbo.t_File f INNER JOIN dbo.t_RegistersAccounts a ON
			f.id=a.rf_idFiles
					INNER JOIN dbo.t_RecordCasePatient r ON
			a.id=r.rf_idRegistersAccounts
...
--		@i TINYINT=0
--declare exec_cursor cursor for SELECT DISTINCT codeM,filialCode FROM #tFile ORDER BY filialCode,CodeM
--open exec_cursor
--fetch next from exec_cursor into @codeM,@i
--while @@fetch_status=0
--begin
--	select LPU,Account, DateAccount, idRecordCase, DateBegin,  DateEnd,  AmountPayment,  V006,  FIO,  BirthDay,  Sex,  SNILS,
--  ENP,  TypePolis,  NumberPolis,  DS1,  DS2,  RSLT,  ISHOD FROM #tFile WHERE CodeM=@CodeM
--	fetch next from exec_cursor into @codeM,@i
--end
--close exec_cursor
