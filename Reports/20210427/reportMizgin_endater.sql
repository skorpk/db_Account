...
count(distinct CASE WHEN m.MUSurgery IN('A16.04.021.004','A16.04.021.003') THEN c.rf_idCase ELSE NULL END) AS Col3,
count(distinct CASE WHEN m.MUSurgery IN('A16.04.021.002', 'A16.04.021.006', 'A16.04.021.011') THEN c.rf_idCase ELSE NULL END) AS Col4,
count(distinct CASE WHEN d.DiagnosisCode IN('T84.5','T84.6','T84.7','T85.7') THEN c.rf_idCase ELSE NULL END) AS Col5
FROM #tCases c LEFT JOIN dbo.t_Meduslugi m ON
		m.rf_idCase = c.rf_idCase
			LEFT JOIN dbo.t_Diagnosis d ON
       d.rf_idCase = c.rf_idCase
