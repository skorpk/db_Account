...
WHERE f.DateRegistration>@dateStart AND f.DateRegistration<@dateEnd  AND a.ReportMonth =@reportMonth AND a.ReportYear=@reportYear
		AND c.rf_idV006=3	 AND m.MUGroupCode=2 AND m.MUUnGroupCode=88 AND m.MUCode BETWEEN 52 AND 103	AND c.Age>17 
...
UNION ALL
SELECT 3, SUM(AmountPayment) AS Col5,COUNT(DISTINCT enp) AS Col6, SUM(AmmPay) AS Col7 FROM #tCases WHERE AmountPaymentAcc>0	AND Sex='Ì' AND Age=65
UNION ALL
...
UNION ALL
SELECT 6, SUM(AmountPayment) AS Col5,COUNT(DISTINCT enp) AS Col6, SUM(AmmPay) AS Col7 FROM #tCases WHERE AmountPaymentAcc>0	AND Sex='Æ' AND Age=65
UNION ALL
