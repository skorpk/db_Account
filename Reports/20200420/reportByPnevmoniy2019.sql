USE AccountOMS
GO
DECLARE @dateStartReg DATETIME='20191001',
...
WHERE f.DateRegistration>=@dateStartReg AND f.DateRegistration<@dateEndReg  AND a.ReportYear=@reportYear AND c.rf_idV006<3  AND a.ReportMonth>9 AND a.ReportMonth<=@reportMonth 
		AND d.TypeDiag=1
...
FROM #tCases p INNER JOIN (SELECT c.rf_idCase,SUM(c.AmountDeduction) AS AmountDeduction
								...
								WHERE c.DateRegistration>=@dateStartReg AND c.DateRegistration<'20200122' 
								GROUP BY c.rf_idCase
							) r ON
			p.rf_idCase=r.rf_idCase

SELECT 2019,c.DS1,COUNT(c.rf_idCase) AS CountCase,cast(SUM(c.AmountPayment) AS money) AS AmountPayment
FROM #tCases c 
