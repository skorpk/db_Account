...
declare @dateBeg datetime='20120926',
		@dateEnd datetime='20120926 23:59:59',
		@codeFilial tinyint=2
...
select * from #t		
select Account,rf_idSMO,sum(Quantity*Tariff_FF) as SumFF from #t group by Account,rf_idSMO
...
from #t t inner join dbo.vw_unitName u on
		t.unitCode=u.unitCode
			inner join (select Account,rf_idSMO,sum(Quantity*Tariff_FF) as SumFF from #t group by Account,rf_idSMO) t1 on
